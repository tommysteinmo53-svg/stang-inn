...
export async function scanHockeyLiveMatchSquads(games:AvailabilityGame[],players:FantasyRosterPlayer[]){
 ...
 const diagnostics:{gameId:string;gameType:string;game:string;matchId:number|null;status:string;homeMembers?:number;awayMembers?:number;missing?:number;error?:string}[]=[];
 for(const game of games){const matchId=Number(game.match_id);if(!Number.isInteger(matchId)||matchId<=0){diagnostics.push({gameId:game.id,gameType:game.game_type,game:`${game.home_team} – ${game.away_team}`,matchId:null,status:"skipped-no-match-id"});continue}
  try{const bundle=await fetchNifMatchBundle(matchId);if(!bundle.availability.teamMembers){diagnostics.push({gameId:game.id,gameType:game.game_type,game:`${game.home_team} – ${game.away_team}`,matchId,status:"skipped-team-members-unavailable"});continue}
   const hk=canonicalTeam(game.home_team),ak=canonicalTeam(game.away_team),homeMembers=bundle.teamMembers.filter(m=>canonicalTeam(rowTeam(m))===hk),awayMembers=bundle.teamMembers.filter(m=>canonicalTeam(rowTeam(m))===ak);
   if(homeMembers.length<10||awayMembers.length<10){diagnostics.push({gameId:game.id,gameType:game.game_type,game:`${game.home_team} – ${game.away_team}`,matchId,status:"skipped-incomplete-lineup",homeMembers:homeMembers.length,awayMembers:awayMembers.length});continue}
   ...
   diagnostics.push({gameId:game.id,gameType:game.game_type,game:`${game.home_team} – ${game.away_team}`,matchId,status:"scanned",homeMembers:homeMembers.length,awayMembers:awayMembers.length,missing});
