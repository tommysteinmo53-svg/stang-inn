...
const statusFor=(sentence:string,name:string):NittenBacktestStatus|null=>{const clause=clauseFor(sentence,name),s=clause.toLocaleLowerCase("nb-NO"),n=name.toLocaleLowerCase("nb-NO"),at=s.indexOf(n);if(at<0)return null;const before=s.slice(Math.max(0,at-110),at),after=s.slice(at+n.length,Math.min(s.length,at+n.length+140));const beforeWide=s.slice(Math.max(0,at-220),at),afterWide=s.slice(at+n.length,Math.min(s.length,at+n.length+280));const local=`${before} ${n} ${after}`;
