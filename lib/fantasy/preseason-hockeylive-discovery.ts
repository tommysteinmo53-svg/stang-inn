...
const PRESEASON_TOURNAMENT_IDS=[448939];
...
type Candidate={matchId:number;text:string;url:string};
...
function candidatesFromHtml(html:string,sourceUrl:string):Candidate[]{
 ...
 while((m=rx.exec(html))){
  ...
  const from=Math.max(0,m.index-1400),to=Math.min(html.length,m.index+1400);
  const snippet=decodeHtml(html.slice(from,to));
  const current=out.get(id);if(!current||snippet.length>current.text.length)out.set(id,{matchId:id,text:snippet,url:sourceUrl});
 }
 return [...out.values()];
}
...
function candidateMatches(game:DbGame,c:Candidate){
 const t=ascii(c.text),home=teamKey(game.home_team),away=teamKey(game.away_team);
 if(!home||!away)return false;
 const hasHome=t.includes(home)||norm(t).includes(home);
 const hasAway=t.includes(away)||norm(t).includes(away);
...
async function discoverOne(game:DbGame){
 ...
 for(const tournamentId of PRESEASON_TOURNAMENT_IDS){
  pages.push(`https://live.hockey.no/?seasonId=${SEASON_ID}&tournamentId=${tournamentId}&matchDate=${encodeURIComponent(game.game_date+"T00:00:00")}`);
 ...
 for(const url of pages){
  try{for(const c of candidatesFromHtml(await fetchHtml(url),url))all.set(c.matchId,c)}catch{/* discovery must never block normal import */}
 }
 const matches=[...all.values()].filter(c=>candidateMatches(game,c));
 if(matches.length!==1)return{status:matches.length===0?"not_found":"ambiguous" as const,matches:matches.map(x=>x.matchId)};
 return{status:"matched" as const,matchId:matches[0].matchId,sourceUrl:matches[0].url,matches:[matches[0].matchId]};
}
...
export async function discoverMissingPreseasonHockeyLiveIds(){
 ...
 for(const game of rows){
  ...
  results.push({gameId:game.id,game:`${game.home_team} – ${game.away_team}`,status:"matched",matchId:found.matchId});
 }
