...
// HockeyLive has used several response wrappers for this endpoint. Walk the payload and
// choose the largest array whose rows look like matches instead of assuming payload.data.matches.
...
export async function prepareFantasySeason(tournamentId:string,season:string){
 ...
 return{tournamentId,season,totalMatches:valid.length,finishedMatches:finishedIds.length,matchIds:finishedIds,sourceRows:matches.length,sampleFields:Object.keys(matches[0]||{}).slice(0,24)}
}
