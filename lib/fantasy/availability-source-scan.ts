...
export const AVAILABILITY_SOURCES:AvailabilitySource[]=[
 ...
 // Local editorial sources. These are discovery-only and are subject to the same
 // freshness, exact-player-name, roster-match and explicit admin-review gates.
...
function publishedAt(html:string){const candidates=[/<meta[^>]+(?:property|name)=["'](?:article:published_time|datePublished|date|publish-date)["'][^>]+content=["']([^"']+)["']/i,/<meta[^>]+content=["']([^"']+)["'][^>]+(?:property|name)=["'](?:article:published_time|datePublished|date|publish-date)["']/i,/"datePublished"\s*:\s*"([^"]+)"/i,/<time[^>]+datetime=["']([^"']+)["']/i,/Publisert:\s*(\d{4}-\d{2}-\d{2})/i];for(const re of candidates){const m=html.match(re);if(m){const d=new Date(m[1]);if(!Number.isNaN(d.getTime()))return d.toISOString()}}return null}
function isFresh(iso:string){return Date.now()-new Date(iso).getTime()<=MAX_SOURCE_AGE_DAYS*86400000}
...
async function getHtml(url:string){const r=await fetch(url,{headers:{"User-Agent":"StangInnAvailabilityBot/1.1 (+admin-reviewed fantasy availability)",Accept:"text/html,application/xhtml+xml"},cache:"no-store",signal:AbortSignal.timeout(10000)});if(!r.ok)throw new Error(`${r.status} ${r.statusText}`);if(!(r.headers.get("content-type")||"").includes("text/html"))throw new Error("ikke HTML");return await r.text()}

export async function scanAvailabilitySources(players:RosterPlayer[],sources=AVAILABILITY_SOURCES){const findings:ScannedFinding[]=[],diagnostics:{source:string;articles:number;matches:number;staleOrUndated:number;error?:string}[]=[];for(const source of sources){let articleCount=0,staleOrUndated=0;const start=findings.length;try{const home=await getHtml(source.url),links=linksFromHtml(home,source.url,source.articlePath),targets=links.length?links:[source.url];for(const url of targets){articleCount++;let html:string;try{html=url===source.url?home:await getHtml(url)}catch{continue}const published=publishedAt(html);if(!published||!isFresh(published)){staleOrUndated++;continue}const text=textFromHtml(html),lower=norm(text);if(!/(skad|syk|karantene|suspend|ute|mangler|tilbake|comeback|usikker|tropp)/.test(lower))continue;for(const p of players){const name=norm(p.name),hit=lower.search(new RegExp(`(^|[^\\p{L}])${escapeRe(name)}([^\\p{L}]|$)`,"u"));if(hit<0)continue;const snippet=lower.slice(Math.max(0,hit-220),Math.min(lower.length,hit+name.length+260)).trim(),status=classify(snippet);if(!status)continue;const rawTeam=source.teamHint||p.team||null,match=matchAvailabilityFinding(p.name,rawTeam,players);findings.push({sourceKind:source.kind,sourceLabel:source.label,sourceUrl:url,sourcePublishedAt:published,rawPlayerName:p.name,rawTeam,rawStatus:status,rawNote:snippet.slice(0,700),proposedPlayerId:match.proposedPlayerId,matchMethod:match.matchMethod,matchConfidence:match.matchConfidence,matchReason:`Automatisk kildefunn fra fersk ${source.kind==="local_media"?"lokalavis":"kilde"}. ${match.matchReason}`,reviewStatus:match.reviewStatus})}}diagnostics.push({source:source.label,articles:articleCount,matches:findings.length-start,staleOrUndated})}catch(e:any){diagnostics.push({source:source.label,articles:articleCount,matches:findings.length-start,staleOrUndated,error:e?.message||"ukjent feil"})}}return{findings,diagnostics}}
