...
export const AVAILABILITY_SOURCES:AvailabilitySource[]=[
  ...
  {kind:"club",label:"Narvik Hockey",url:"https://www.narvikhockey.no/",teamHint:"Narvik Hockey"},
  {kind:"club",label:"Nidaros Hockey",url:"https://www.nidaroshockey.no/",teamHint:"Nidaros Hockey"},
...
function linksFromHtml(html:string,base:string,articlePath?:RegExp){
  ...
  while((m=re.exec(html))){
    try{
      ...
      if(articlePath&&!articlePath.test(u.pathname))continue;
      if(!articlePath){
        if(u.pathname==="/"||u.pathname.length<8||/\.(jpg|jpeg|png|gif|svg|pdf|mp4)$/i.test(u.pathname))continue;
        if(/\/(kontakt|billetter|statistikk|kamper|lag|team|shop|webshop|personvern|cookie|sponsor)/i.test(u.pathname))continue;
      }
      u.hash="";
...
async function getHtml(url:string){
  const r=await fetch(url,{headers:{"User-Agent":"StangInnAvailabilityBot/1.0 (+admin-reviewed fantasy availability)"},cache:"no-store",signal:AbortSignal.timeout(10000)});
  if(!r.ok)throw new Error(`${r.status} ${r.statusText}`);
  const type=r.headers.get("content-type")||"";
  if(!type.includes("text/html"))throw new Error("ikke HTML");
  return await r.text();
}
...
export async function scanAvailabilitySources(players:RosterPlayer[],sources=AVAILABILITY_SOURCES){
  ...
  const diagnostics:{source:string;articles:number;matches:number;error?:string}[]=[];
  for(const source of sources){
    let articleCount=0;
    const start=findings.length;
    try{
      const home=await getHtml(source.url);
      const links=linksFromHtml(home,source.url,source.articlePath);
      const targets=links.length?links:[source.url];
      for(const url of targets){
        articleCount++;
        let html:string;
        try{html=url===source.url?home:await getHtml(url)}catch{continue}
        const text=textFromHtml(html);
        const lower=norm(text);
        if(!/(skad|syk|karantene|suspend|ute|mangler|tilbake|comeback|usikker|tropp)/.test(lower))continue;
        for(const p of players){
          const name=norm(p.name);
          const hit=lower.search(new RegExp(`(^|[^\\p{L}])${escapeRe(name)}([^\\p{L}]|$)`,"u"));
          if(hit<0)continue;
          const a=Math.max(0,hit-220),b=Math.min(lower.length,hit+name.length+260);
          const snippet=text.slice(a,b).replace(/\s+/g," ").trim();
          const status=classify(snippet);
          if(!status)continue;
          const rawTeam=source.teamHint||p.team||null;
          const match=matchAvailabilityFinding(p.name,rawTeam,players);
          findings.push({sourceKind:source.kind,sourceLabel:source.label,sourceUrl:url,rawPlayerName:p.name,rawTeam,rawStatus:status,rawNote:snippet.slice(0,700),proposedPlayerId:match.proposedPlayerId,matchMethod:match.matchMethod,matchConfidence:match.matchConfidence,matchReason:`Automatisk kildefunn. ${match.matchReason}`,reviewStatus:match.reviewStatus});
        }
      }
      diagnostics.push({source:source.label,articles:articleCount,matches:findings.length-start});
    }catch(e:any){
      diagnostics.push({source:source.label,articles:articleCount,matches:findings.length-start,error:e?.message||"ukjent feil"});
    }
  }
