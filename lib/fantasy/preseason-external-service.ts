...
type RegistryStat={playerName:string;team:string;position?:string|null;didPlay?:boolean|null;goals?:number;assists?:number;shots?:number;plusMinus?:number;pim?:number;saves?:number;goalsAgainst?:number;minutesPlayed?:number;win?:boolean|null;shutout?:boolean|null;powerplayGoals?:number;powerplayAssists?:number;shorthandedGoals?:number;shorthandedAssists?:number;knownFields:string[];sourceQuality:number};
type LineupPlayer={number?:number;playerName:string;position?:string|null;line?:number;extra?:boolean};
type RegistryGame={gameDate:string;homeTeam:string;awayTeam:string;homeScore?:number|null;awayScore?:number|null;status:"scheduled"|"finished"|"cancelled";sourceType:"official"|"web"|"manual";sourceQuality:number;sources:{url:string;label:string;role:string}[];stats:RegistryStat[];lineup?:Record<string,LineupPlayer[]>;notes?:string|null};
...
function expandedStats(game:RegistryGame){
 const byKey=new Map<string,RegistryStat>();
 for(const stat of game.stats||[])byKey.set(`${canonicalTeam(stat.team)}:${norm(stat.playerName)}`,{...stat});
 for(const [teamName,lineup] of Object.entries(game.lineup||{}))for(const lp of lineup||[]){const key=`${canonicalTeam(teamName)}:${norm(lp.playerName)}`;const existing=byKey.get(key);if(existing){existing.knownFields=Array.from(new Set([...(existing.knownFields||[]),"lineup"]));continue}
   const isGoalie=String(lp.position||"").toUpperCase()==="G";
   byKey.set(key,{playerName:lp.playerName,team:teamName,position:lp.position||null,didPlay:isGoalie?false:true,knownFields:isGoalie?["lineup"]:["lineup","didPlay"],sourceQuality:game.sourceQuality});
 }
 return [...byKey.values()];
}

function rawKnown(stat:RegistryStat,game:RegistryGame,match:MatchResult){return{source:"external-preseason-registry",knownFields:stat.knownFields,sources:game.sources,match:{confidence:match.confidence,reason:match.reason},game:{gameDate:game.gameDate,homeTeam:game.homeTeam,awayTeam:game.awayTeam}}}

export async function importExternalPreseasonRegistry(){
 const sb=serverClient(),players=await loadFantasyPlayers(sb),results:any[]=[];
 for(const game of (registry.games as RegistryGame[])){
  const{data:dbGame,error:gameError}=await sb.from("fantasy_preseason_games").select("*").eq("season",registry.season).eq("game_date",game.gameDate).eq("home_team",game.homeTeam).eq("away_team",game.awayTeam).maybeSingle();if(gameError)throw gameError;if(!dbGame){results.push({ok:false,game:`${game.homeTeam} - ${game.awayTeam}`,error:"Kampen finnes ikke i fantasy_preseason_games"});continue}
  const primarySource=game.sources.find(s=>s.role==="official_report")||game.sources.find(s=>s.role==="official_events")||game.sources.find(s=>s.role==="official_lineup")||game.sources.find(s=>s.role==="official_schedule")||game.sources[0]||null;
  const patch:any={home_score:game.homeScore??dbGame.home_score,away_score:game.awayScore??dbGame.away_score,status:game.status,source_type:game.sourceType,source_quality:Math.max(Number(dbGame.source_quality||0),Number(game.sourceQuality||0)),source_url:primarySource?.url||dbGame.source_url,notes:[dbGame.notes,game.notes,`External sources: ${game.sources.map(s=>`${s.label} (${s.role})`).join(" | ")}`].filter(Boolean).join("\n"),updated_at:new Date().toISOString()};const{error:patchError}=await sb.from("fantasy_preseason_games").update(patch).eq("id",dbGame.id);if(patchError)throw patchError;
  let matched=0,unmatched=0,rows=0,strong=0;for(const stat of expandedStats(game)){const match=matchPlayer(stat,players),player=match.player;const row={preseason_game_id:dbGame.id,player_id:player?.id||null,raw_player_name:stat.playerName,team:stat.team,position:player?.position||stat.position||null,did_play:stat.didPlay??null,goals:Number(stat.goals||0),assists:Number(stat.assists||0),shots:Number(stat.shots||0),plus_minus:Number(stat.plusMinus||0),pim:Number(stat.pim||0),saves:Number(stat.saves||0),goals_against:Number(stat.goalsAgainst||0),minutes_played:Number(stat.minutesPlayed||0),win:stat.win??null,shutout:stat.shutout??null,powerplay_goals:Number(stat.powerplayGoals||0),powerplay_assists:Number(stat.powerplayAssists||0),shorthanded_goals:Number(stat.shorthandedGoals||0),shorthanded_assists:Number(stat.shorthandedAssists||0),source_type:game.sourceType,source_quality:Number(stat.sourceQuality||game.sourceQuality||0.5),raw:rawKnown(stat,game,match),updated_at:new Date().toISOString()};const{error}=await sb.from("fantasy_preseason_player_stats").upsert(row,{onConflict:"preseason_game_id,raw_player_name,team"});if(error)throw error;rows++;if(player){matched++;if(match.confidence==="strong")strong++}else unmatched++}
...
export async function diagnoseExternalPreseasonMatches(){const sb=serverClient(),players=await loadFantasyPlayers(sb),rows:any[]=[];for(const game of (registry.games as RegistryGame[]))for(const stat of expandedStats(game)){const match=matchPlayer(stat,players);rows.push({gameDate:game.gameDate,game:`${game.homeTeam} - ${game.awayTeam}`,rawPlayerName:stat.playerName,rawTeam:stat.team,matched:!!match.player,confidence:match.confidence,reason:match.reason,matchedPlayer:match.player?{id:match.player.id,name:match.player.name,team:match.player.team,position:match.player.position}:null,candidates:match.candidates.map(p=>({id:p.id,name:p.name,team:p.team,position:p.position}))})}return{total:rows.length,matched:rows.filter(r=>r.matched).length,unmatched:rows.filter(r=>!r.matched).length,strong:rows.filter(r=>r.confidence==="strong").length,rows}}
export function getExternalPreseasonRegistrySummary(){return{season:registry.season,scheduleReference:registry.scheduleReference,games:(registry.games||[]).map((g:any)=>({gameDate:g.gameDate,homeTeam:g.homeTeam,awayTeam:g.awayTeam,sourceQuality:g.sourceQuality,statRows:expandedStats(g as RegistryGame).length,sources:g.sources||[]}))}}
