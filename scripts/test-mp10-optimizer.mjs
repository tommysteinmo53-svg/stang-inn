...
const fullOptimizerRoute=read("app/api/admin/fantasy/optimizer/route.ts");
const adminAuth=read("lib/fantasy/admin-auth.ts");
...
must("shared admin guard validates players.admin",adminAuth.includes('.from("players")')&&adminAuth.includes('.select("admin")')&&adminAuth.includes('!player?.admin'));
must("full-team optimizer uses server-side admin guard",fullOptimizerRoute.includes('requireFantasyAdmin')&&fullOptimizerRoute.includes('const admin=await requireFantasyAdmin(request)'));
must("transfer optimizer uses server-side admin guard",route.includes('requireFantasyAdmin')&&route.includes('const admin=await requireFantasyAdmin(request)'));
