...
export async function GET(request:NextRequest){
  ...
  try{
    ...
    const {data:stats,error:statsError}=await supabase.from("fantasy_player_game_stats").select("player_id,goals,assists,shots,plus_minus,pim,saves,goals_against,win,shutout,did_play,position_snapshot,team_snapshot").eq("game_id",game.id);
    ...
    const rows=(stats??[]).map((s:any)=>{const p:any=byId.get(s.player_id)||{};return{name:p.name||"Ukjent",team:s.team_snapshot||p.team||"",position:s.position_snapshot||p.position||"",goals:s.goals||0,assists:s.assists||0,shots:s.shots||0,plusMinus:s.plus_minus||0,pim:s.pim||0,saves:s.saves||0,goalsAgainst:s.goals_against||0,win:s.win,shutout:s.shutout,didPlay:s.did_play};}).sort((a:any,b:any)=>a.team.localeCompare(b.team)||a.position.localeCompare(b.position)||a.name.localeCompare(b.name));
