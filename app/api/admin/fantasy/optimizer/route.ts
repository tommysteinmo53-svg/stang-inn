...
export async function GET(request:NextRequest){
  ...
  const[{data:economy,error:economyError},{data:xfp,error:xfpError},{data:purchase,error:purchaseError},{data:approvedAvailability,error:approvedAvailabilityError}]=await Promise.all([
    userSb.rpc("get_fantasy_economy_admin_v1",{p_season:"2026/27"}),userSb.rpc("get_fantasy_xfp_admin_v1",{p_season:"2026/27"}),serviceSb.from("fantasy_players").select("id,active,on_current_roster,available_for_purchase"),serviceSb.from("fantasy_player_availability").select("player_id,status")]);
