...
export async function POST(request:NextRequest){const admin=await requireFantasyAdmin(request);if(!admin.ok)return admin.response;try{const c=sb(),now=Date.now(),today=new Date(now).toISOString().slice(0,10),cutoffDate=new Date(now-3*86400000).toISOString().slice(0,10),fromIso=new Date(now-3*86400000).toISOString(),toIso=new Date(now+86400000).toISOString();
 ...
 const games:AvailabilityGame[]=[...(preseason||[]).map((g:any)=>({id:`pre:${g.id}`,game_date:g.game_date,home_team:g.home_team,away_team:g.away_team,match_id:Number(g.hockeylive_match_id)||null,source_url:g.source_url||null,game_type:"preseason" as const})),...(regular||[]).map((g:any)=>({id:`reg:${g.id}`,game_date:String(g.starts_at).slice(0,10),home_team:g.home_team,away_team:g.away_team,match_id:numericMatchId(g.external_id),source_url:null,game_type:"regular" as const}))];
