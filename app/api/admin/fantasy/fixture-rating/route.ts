...
export async function GET(request:NextRequest){
  ...
  const{data,error}=await sb.rpc("get_fantasy_fixture_rating_admin_v3",{p_season:"2026/27"});
  if(error)return NextResponse.json({ok:false,error:error.message},{status:500});

  return NextResponse.json({ok:true,rows:data||[],definition:{
    version:"v3",scale:"1–5",direction:"1 = svært vanskelig, 5 = svært lett",
    thresholds:{very_hard:"≤ 0.85",hard:"0.851–0.95",neutral:"0.951–1.049",easy:"1.05–1.149",very_easy:"≥ 1.15"},
    transition:"Historisk 2025/26 EHL-baseline brukes ved sesongstart. Live 2026/27-data fases inn lineært over lagets første 12 ferdigspilte seriekamper og er 100 % vektet fra kamp 12.",
    baseline:"Baselinen er låst fra alle 225 ordinære EHL-kamper 2025/26. Utespillere bruker motstanderens GA relativt til ligasnitt; keepere bruker motstanderens GF. Ringerike starter nøytralt (1,000) fordi laget mangler 2025/26 EHL-baseline.",
    ...
    source:"fantasy_xfp_opponent_factor",preseasonStatsUsed:false,
  }});
}
