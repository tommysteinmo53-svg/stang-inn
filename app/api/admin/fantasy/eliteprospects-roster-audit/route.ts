...
function candidateSet(ep:any,pool:any[]){
  ...
  if(candidates.length===0){
    const reviewedId=REVIEWED_EP_TO_NIF_2026[norm(ep.name)];
    if(reviewedId){candidates=pool.filter((p:any)=>String(p.external_id??"")===reviewedId);matchMethod="reviewed_ep_nif_alias"}
  }
