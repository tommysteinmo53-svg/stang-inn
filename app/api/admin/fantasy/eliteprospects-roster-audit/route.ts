...
function candidateSet(ep:any,pool:any[]){
  let candidates=pool.filter((p:any)=>norm(p.name)===norm(ep.name));let matchMethod="exact_normalized_name";
  ...
  try{
    ...
    for(const ep of ELITEPROSPECTS_ROSTER_2026){
      ...
      if(candidates.length===1){
        const p=candidates[0],dbTeam=canonicalFantasyTeam(p.team);const row={epName:ep.name,epTeam:ep.team,epRole:ep.role,playerId:p.id,dbName:p.name,dbTeam,rawDbTeam:p.team,position:p.position,externalId:p.external_id,matchMethod};
        if(!roleCompatible(ep.role,p.position))positionMismatch.push({...row,reason:"position_role_mismatch"});
        if(dbTeam===ep.team){used.add(String(p.id));if(matchMethod==="exact_normalized_name")matched.push(row);else variantMatched.push(row)}else teamMismatch.push(row);
      }else if(candidates.length>1){ambiguous.push({epName:ep.name,epTeam:ep.team,epRole:ep.role,reason:matchMethod==="exact_normalized_name"?"duplicate_normalized_name":"multiple_deterministic_name_variants",matchMethod,candidates:candidates.map(playerBrief)})}
    ...
    // Reconciliation preview: resolve EP against every historical fantasy_players row before proposing writes.
