...
import {discoverMissingPreseasonHockeyLiveIds} from "../../../../../lib/fantasy/preseason-hockeylive-discovery";
...
export async function POST(request:NextRequest){
 ...
 try{
  ...
  if(body?.all===true){
   const discovery=await discoverMissingPreseasonHockeyLiveIds();
   const result=await importAllPreseasonHockeyLive();
   const scoreReconcile=await reconcileHockeyLivePreseasonScores();
   return NextResponse.json({ok:true,...result,discovery,scoreReconcile});
  }
