...
export async function POST(request:NextRequest){
 ...
 try{
  ...
  const{data:games,error:ge}=await db.from("fantasy_games").select("id,home_team,away_team,home_score,away_score,season").eq("season",season);if(ge)throw ge;
  ...
  for(const p of players||[]){
   ...
   results.push({name:p.name,team:p.team,targetSO:target,candidates:candidates.length,applied,secondsFixed:secondsUpdates.length,candidateGameIds:candidates.map((s:any)=>s.game_id)});
  }
