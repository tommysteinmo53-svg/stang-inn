...
async function requireAdmin(request: NextRequest) {
  ...
  if (!supabaseUrl || !publishableKey) {
    return { ok: false as const, response: NextResponse.json({ ok: false, error: "Supabase public-konfigurasjon mangler." }, { status: 503 }) };
  }

  const authHeader = request.headers.get("authorization");
  const token = authHeader?.startsWith("Bearer ") ? authHeader.slice(7) : null;
  if (!token) {
    return { ok: false as const, response: NextResponse.json({ ok: false, error: "Mangler innlogging." }, { status: 401 }) };
  }
  ...
  if (userError || !userData.user) {
    return { ok: false as const, response: NextResponse.json({ ok: false, error: "Ugyldig innlogging." }, { status: 401 }) };
  }
  ...
  if (!player?.admin) {
    return { ok: false as const, response: NextResponse.json({ ok: false, error: "Kun admin kan kjøre Fantasy-snapshot." }, { status: 403 }) };
  }
