...
function AwardCard({ award, variant = "standard" }: { award: Award; variant?: "featured" | "standard" | "miss" }) {
  return <article className={`awardCard awardCardPolished ${variant} ${award.player ? "hasWinner" : "pendingAward"}`}>
    <div className="awardCardTop"><div className="awardIcon">{award.icon}</div><span className="awardState">{award.player ? "Kåret" : "Venter"}</span></div>
...
export default function AwardsPage() {
  ...
  return <main className="appShell awardsPage"><header className="topbar"><a href="/" className="brand brandButton" style={{textDecoration:"none"}}><div className="brandMark">🏒</div><div><p className="eyebrow">Hall of fame</p><h1>Awards</h1></div></a><a href="/" className="textButton">← Hjem</a></header>
    <section className="pageStack awardsStack"><article className="heroCard awardsHero"><div><p className="eyebrow">Prestasjoner</p><h2>Sesongens kåringer</h2><p className="muted">Rundevinnere, eksperter, streaks og de største bommene – automatisk basert på faktiske tippingresultater.</p></div><div className="countdown"><strong>{awardedCount}/7</strong><span>kåringer aktive</span></div></article>
      ...
      <div className="awardsSectionHeading missHeading"><div><p className="eyebrow">På den andre siden …</p><h3>Bomkåringene</h3></div><span className="statusPill">😬 Hederlig omtale</span></div><section className="awardsMissGrid">{misses.map((award)=><AwardCard key={award.title} award={award} variant="miss"/>)}</section>
      <article className="panel awardsRules"><div className="panelHeading"><div><p className="eyebrow">Slik fungerer det</p><h3>Kåringene følger faktiske resultater</h3></div><span className="statusPill">Automatisk</span></div><div className="awardsRuleGrid"><div><strong>📅 Månedsvinner</strong><span>Kåres først når kalendermåneden er avsluttet.</span></div><div><strong>🧠 Eksperttittel</strong><span>Krever tips på minst 75 % av ferdigspilte kamper.</span></div><div><strong>🤦 Ukens bom</strong><span>Bruker kun siste fullførte EHL-runde.</span></div></div><p className="muted awardsEngineNote">Poengbaserte kåringer bruker kun poeng scoret av den autoritative tippingmotoren.</p></article>
    </section>
  </main>;
}
