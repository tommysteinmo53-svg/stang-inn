...
function monthLabel(key: string) {
  const [year, month] = key.split("-").map(Number);
  const label = new Intl.DateTimeFormat("no-NO", { month: "long", year: "numeric" }).format(new Date(year, month - 1, 1, 12));
  return label.charAt(0).toUpperCase() + label.slice(1);
}

function missDistance(match: Match, tip: Tip) {
  return Math.abs(tip.home_tip - match.home_score!) + Math.abs(tip.away_tip - match.away_score!);
}
...
export default function AwardsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [playerResult, matchResult, tipResult] = await Promise.all([
        ...
        supabase.from("matches").select("id,home_team,away_team,home_score,away_score,finished,round,match_time").order("match_time"),
  ...
  const data = useMemo(() => {
    ...
    const summarize = (player: Player, rows: Tip[]) => {
      let exact = 0, correct = 0, points = 0;
    ...
    if (scored.length) {
      for (const player of players) {
        ...
        for (const match of [...finished].sort((a, b) => (a.match_time || "").localeCompare(b.match_time || ""))) {
          const tip = scored.find((candidate) => candidate.player_id === player.id && candidate.match_id === match.id);
          if (tip && pointsFor(tip) > 0) { current += 1; best = Math.max(best, current); } else current = 0;
        }
        if (best > 0 && (!streak || best > streak.n)) streak = { p: player, n: best };
      }
    }

    const roundNumbers = [...new Set(matches.map((match) => match.round).filter((round): round is number => round !== null))].sort((a, b) => a - b);
    ...
    if (latestRound !== undefined) {
      ...
      roundWinner = players.map((player) => summarize(player, scored.filter((tip) => tip.player_id === player.id && roundIds.has(tip.match_id))))
        .sort((a, b) => b.points - a.points || b.exact - a.exact || b.correct - a.correct || a.p.display_name.localeCompare(b.p.display_name, "no"))
        .find((row) => row.tipped > 0) || null;
    }

    const closedScoredMonths = [...new Set(scored.map((tip) => monthKey(matchMap.get(tip.match_id)?.match_time || null)).filter((key): key is string => key !== null && monthIsClosed(key)))].sort();
    ...
    if (latestClosedMonth) {
      const monthIds = new Set(finished.filter((match) => monthKey(match.match_time) === latestClosedMonth).map((match) => match.id));
      monthlyWinner = players.map((player) => summarize(player, scored.filter((tip) => tip.player_id === player.id && monthIds.has(tip.match_id))))
        .filter((row) => row.tipped > 0)
        .sort((a, b) => b.points - a.points || b.exact - a.exact || b.correct - a.correct || a.p.display_name.localeCompare(b.p.display_name, "no"))[0] || null;
    }

    const chooseMiss = (rows: Tip[]): Miss | null => {
      const candidates = rows.map((tip) => {
        const match = matchMap.get(tip.match_id);
        const player = players.find((candidate) => candidate.id === tip.player_id);
        return match && player ? { p: player, distance: missDistance(match, tip), match, tip } : null;
      }).filter((row): row is Miss => row !== null);
      return candidates.sort((a, b) => b.distance - a.distance || (a.match.match_time || "").localeCompare(b.match.match_time || "") || a.p.display_name.localeCompare(b.p.display_name, "no") || a.match.id - b.match.id)[0] || null;
    };

    const weeklyRoundIds = latestRound === undefined ? new Set<number>() : new Set(finished.filter((match) => match.round === latestRound).map((match) => match.id));
    const weeklyMiss = latestRound === undefined ? null : chooseMiss(finishedTips.filter((tip) => weeklyRoundIds.has(tip.match_id)));
    const seasonMiss = chooseMiss(finishedTips);
    ...
    const awards: Award[] = [
      ...
      { icon: "💥", title: "Sesongens bom", player: seasonMiss?.p || null, value: seasonMiss ? `${seasonMiss.tip.home_tip}–${seasonMiss.tip.away_tip}` : "–", detail: seasonMiss ? `${seasonMiss.match.home_team}–${seasonMiss.match.away_team} endte ${seasonMiss.match.home_score}–${seasonMiss.match.away_score}` : "Ingen ferdige kamper" },
    ];
    return awards;
  ...
  return <main className="appShell">
    <header className="topbar"><a href="/" className="brand brandButton" style={{ textDecoration: "none" }}><div className="brandMark">🏒</div><div><p className="eyebrow">Hall of fame</p><h1>Awards</h1></div></a></header>
