...
function statusText(match: Match) {
  ...
  if (isLive(match)) {
    if (match.home_score !== null && match.away_score !== null) return `🟢 LIVE ${match.home_score}–${match.away_score}`;
    return "🟢 LIVE";
  }
  return fmt(match.match_time);
}

function featured(match: Match) {
  return `${match.home_team} ${match.away_team}`.toLowerCase().includes("narvik");
}
...
export default function RoundPage() {
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const active = matches.find(m => m.round !== null && !!m.match_time && !m.finished && new Date(m.match_time).getTime() <= now);
    const next = matches.find(m => m.round !== null && (!m.match_time || new Date(m.match_time).getTime() >= now));
    setSelectedRound(active?.round ?? next?.round ?? rounds[0]);
    ...
    <section className="pageStack" style={{ marginTop: 24 }}>
      <article className="heroCard">
        <div>
          ...
          <p className="muted">Oppdateres automatisk hvert 30. sekund. Alle tips blir synlige når den enkelte kampen starter.</p>
        </div>
        <div className="countdown"><strong>{liveMatches.length ? `🟢 ${liveMatches.length}` : completedCount}</strong><span>{liveMatches.length ? "live nå" : `${completedCount}/${roundMatches.length} ferdig`}</span></div>
      </article>
      ...
      {leader && <article className="quoteCard">
        <span>{liveMatches.length ? "🟢 LIVE-RUNDELEDER" : "🏆 RUNDELEDER"}</span>
        <p><strong>{leader.display_name}</strong> leder med {leader.points} poeng{leader.exact ? ` og ${leader.exact} eksakte` : ""}.</p>
      </article>}

      <article className="panel standings">
        <div className="panelHeading">
          <div><p className="eyebrow">Rundetabell</p><h3>Poeng denne runden</h3></div>
          <span className="statusPill">{updatedAt ? `Oppdatert ${updatedAt.toLocaleTimeString("no-NO", { hour: "2-digit", minute: "2-digit" })}` : ""}</span>
        </div>
      ...
      {roundMatches.map(match => {
        ...
        return <article
          className="panel"
          key={match.id}
          style={isFeatured ? { border: "1px solid rgba(245,196,81,.55)", boxShadow: "0 0 0 1px rgba(245,196,81,.08) inset" } : live ? { border: "1px solid rgba(79,214,156,.45)" } : undefined}
        >
          <div className="panelHeading">
            <div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 5 }}>
                {isFeatured && <span className="statusPill">⭐ Rundens kamp</span>}
                {live && <span className="delivery complete">🟢 LIVE</span>}
              </div>
          ...
          {!visible ? <div className="quoteCard"><span>Tips skjult</span><p>🔒 Andre spilleres tips blir synlige ved kampstart.</p></div> : <div className="simpleList">
            {players.map(player => {
              ...
              const exact = match.finished && Number(tip?.points ?? 0) === 5;
              const correct = match.finished && Number(tip?.points ?? 0) === 3;
              return <div key={player.id} style={exact ? { background: "rgba(245,196,81,.08)", borderRadius: 10, paddingInline: 8 } : undefined}>
                <span>{exact ? "🎯 " : correct ? "✓ " : ""}{player.display_name}{player.id === meId ? " · deg" : ""}</span>
                <strong>{tip ? `${tip.home_tip}–${tip.away_tip}${match.finished ? ` · +${Number(tip.points ?? 0)} p${exact ? " · EKSAKT" : ""}` : ""}` : "Ikke levert"}</strong>
              </div>;
            })}
          </div>}
        </article>;
