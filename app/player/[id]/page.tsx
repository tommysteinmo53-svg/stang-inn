...
export default function PlayerProfilePage() {
  ...
  const roundWins = useMemo(()=>{
    const rounds=[...new Set(matches.filter(m=>m.finished&&m.round!=null).map(m=>m.round as number))].sort((a,b)=>a-b);
    let wins=0;
    rounds.forEach(r=>{
      const mids=new Set(matches.filter(m=>m.finished&&m.round===r).map(m=>m.id));
      const rs=players.map(p=>({id:p.id,pts:tips.filter(t=>t.player_id===p.id&&mids.has(t.match_id)).reduce((s,t)=>s+Number(t.points??0),0)})).sort((a,b)=>b.pts-a.pts);
      if(rs[0]?.id===playerId) wins++;
    });
    return wins;
  },[matches,players,tips,playerId]);

  const trend = useMemo<TrendPoint[]>(()=>{
    const rounds=[...new Set(matches.filter(m=>m.finished&&m.round!=null).map(m=>m.round as number))].sort((a,b)=>a-b);
    return rounds.map(r=>{
      const mids=new Set(matches.filter(m=>m.finished&&(m.round??0)<=r).map(m=>m.id));
      const table=players.map(p=>({id:p.id,pts:tips.filter(t=>t.player_id===p.id&&mids.has(t.match_id)).reduce((s,t)=>s+Number(t.points??0),0)})).sort((a,b)=>b.pts-a.pts||a.id.localeCompare(b.id));
  ...
  return <main className="appShell">
    ...
    <section className="pageStack" style={{ marginTop: 24 }}>
      ...
      <section className="statsGrid">
        ...
        <article className="miniCard"><span>🏆 Rundeseire</span><strong>{roundWins}</strong><small>så langt</small></article>
      ...
      <section className="contentGrid">
        <article className="panel"><div className="panelHeading"><div><p className="eyebrow">Form</p><h3>Siste 5 runder</h3></div></div><div className="simpleList">{lastFive.map(r=><div key={r.round}><span>Runde {r.round}</span><strong>{r.points} p · {r.exact} eksakte</strong></div>)}{!lastFive.length&&<p className="muted">Ingen ferdigspilte runder ennå.</p>}</div></article>
