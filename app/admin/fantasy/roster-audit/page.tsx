...
export default function RosterAuditPage(){
  ...
  async function run(){
    ...
    try{
      ...
      setData(j);
      const x=j.summary||{};
      setMessage(`Ferdig · EP ${x.expected??"?"} · aktiv/current ${x.current??"?"} · match ${x.matched??"?"} (${x.exactMatched??"?"} eksakt + ${x.variantMatched??"?"} sikker navnevariant) · mangler ${x.missing??"?"} · lagavvik ${x.teamMismatch??"?"} · tvetydige ${x.ambiguous??"?"} · ekstra ${x.extras??"?"}`);
  ...
  return <main className="appShell">
    ...
    <section className="pageStack" style={{marginTop:24}}>
      <article className="heroCard"><div><p className="eyebrow">Preseason source of truth</p><h2>Produksjonsroster ↔ EliteProspects</h2><p className="muted">Read-only kontroll. Ingen spillerdata endres. Eksakt navn matches først. Deretter tillates bare deterministiske navnevarianter: hele navnetokens må stemme, EP-fornavn og etternavn må finnes i riktig rekkefølge i NIF-navnet, kandidaten må være unik i hele ligaen og posisjonen kompatibel. Ingen fuzzy matching eller automatisk gjetting.</p></div><button onClick={run} disabled={busy}>{busy?"Kontrollerer …":"Kjør full roster-audit"}</button></article>
