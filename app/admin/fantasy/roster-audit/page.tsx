...
export default function RosterAuditPage(){
  ...
  async function run(){
    ...
    try{
      ...
      setData(j);
      const x=j.summary||{};
      setMessage(`Ferdig · EP ${x.expected??"?"} · aktiv/current ${x.current??"?"} · eksakt match ${x.matched??"?"} · mangler ${x.missing??"?"} · lagavvik ${x.teamMismatch??"?"} · tvetydige ${x.ambiguous??"?"} · ekstra ${x.extras??"?"}`);
  ...
  return <main className="appShell">
    ...
    <section className="pageStack" style={{marginTop:24}}>
      <article className="heroCard"><div><p className="eyebrow">Preseason source of truth</p><h2>Produksjonsroster ↔ EliteProspects</h2><p className="muted">Read-only kontroll. Ingen spillerdata endres. Kun eksakt normalisert fullt navn matches automatisk; andre kandidater sendes til manuell identitetskontroll.</p></div><button onClick={run} disabled={busy}>{busy?"Kontrollerer …":"Kjør full roster-audit"}</button></article>
      <article className="panel"><p style={{fontWeight:800}}>{message}</p></article>
      {summary&&<div className="statsGrid">{[["EP-fasit",summary.expected],["Aktiv/current",summary.current],["Eksakt match",summary.matched],["Mangler",summary.missing],["Lagavvik",summary.teamMismatch],["Tvetydige",summary.ambiguous],["Ekstra",summary.extras]].map(([label,value])=><div className="panel" key={String(label)}><p className="eyebrow">{label}</p><div style={{fontSize:28,fontWeight:900}}>{value}</div></div>)}</div>}
      {data?.perTeam&&<article className="panel"><div className="panelHeading"><div><p className="eyebrow">Alle EHL-lag</p><h3>Lagkontroll</h3></div></div><div style={{overflowX:"auto"}}><table style={{width:"100%",minWidth:760,borderCollapse:"collapse"}}><thead><tr>{["Lag","EP","DB current","Match","Mangler","Lagavvik","Ekstra"].map(h=><th key={h} style={{textAlign:"left",padding:8}}>{h}</th>)}</tr></thead><tbody>{data.perTeam.map((r:any)=><tr key={r.team}><td style={{padding:8,fontWeight:800}}>{r.team}</td><td>{r.expected}</td><td>{r.current}</td><td>{r.matched}</td><td>{r.missing}</td><td>{r.teamMismatch}</td><td>{r.extras}</td></tr>)}</tbody></table></div></article>}
      ...
      {data&&<article className="panel"><details open><summary style={{cursor:"pointer",fontWeight:900}}>Komplett audit-resultat</summary><pre style={{marginTop:12,whiteSpace:"pre-wrap",overflowWrap:"anywhere",fontSize:11}}>{JSON.stringify(data,null,2)}</pre></details></article>}
    </section>
  </main>;
}
