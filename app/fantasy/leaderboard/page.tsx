...
type Standing={standings_position:number;previous_standings_position:number|null;position_change:number;participant_count:number;team_id:string;team_name:string;total_points:number;rounds_scored:number;round_wins:number;best_round_points:number;average_round_points:number;last_round_no:number|null;last_round_points:number|null;last_round_position:number|null;last_round_participants:number|null};
...
export default function FantasyLeaderboard(){
 ...
 const myTeam=rows.find(r=>r.team_id===myTeamId),participants=leader?.participant_count??rows.length;
 ...
 return <main className="fantasy-shell leaderboard-shell">
  ...
  <section className="leaderboard-summary">
   <article><small>Leder</small><strong>{hasScoredRounds&&leader?leader.team_name:"—"}</strong><span>{hasScoredRounds&&leader?`${pts(leader.total_points)} poeng`:"Sesongen har ikke startet"}</span></article>
   <article><small>Mitt lag</small><strong>{hasScoredRounds&&myTeam?`${myTeam.standings_position}. av ${participants}`:"—"}</strong><span>{hasScoredRounds&&myTeam?`${movement(myTeam)} siden forrige runde`:myTeam?"Venter på første scorede runde":"Ingen plassering ennå"}</span></article>
   <article><small>Deltakere</small><strong>{participants}</strong><span>{leader?.last_round_no?`Siste scorede: runde ${leader.last_round_no}`:"Ingen scorede runder ennå"}</span></article>
  ...
  <section className="team-panel leaderboard-table-panel"><div className="leaderboard-section-head"><div><p className="eyebrow">SESONGTABELL</p><h2>Stillingen</h2></div><span className="team-muted">{participants} deltakere · trykk på et lag for rundehistorikk</span></div>
   {rows.length===0?<div className="leaderboard-empty"><strong>Ingen fantasy-lag ennå.</strong><span>Tabellen fylles automatisk når lag opprettes og rundene scores.</span></div>:<div className="leaderboard-list">
