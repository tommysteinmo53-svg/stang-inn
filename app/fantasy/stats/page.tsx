...
export default function FantasyStatsPage(){
 ...
 const total=scored.at(-1)?.cumulative_points??0,avg=scored.length?total/scored.length:null,best=scored.length?Math.max(...scored.map(r=>r.round_points||0)):null,worst=scored.length?Math.min(...scored.map(r=>r.round_points||0)):null,currentRank=scored.at(-1)?.overall_rank??null;
 const sortedPoints=scored.map(r=>r.round_points||0).sort((a,b)=>a-b),median=sortedPoints.length?(sortedPoints.length%2?sortedPoints[(sortedPoints.length-1)/2]:(sortedPoints[sortedPoints.length/2-1]+sortedPoints[sortedPoints.length/2])/2):null;
 ...
 <section className="team-panel"><div className="leaderboard-section-head"><div><p className="eyebrow">SESONGINNSIKT</p><h2>Nøkkeltall</h2></div></div><div className="stats-insight-grid"><article><span>Median</span><strong>{pts(median)} p</strong></article><article><span>Verste runde</span><strong>{pts(worst)} p</strong></article><article><span>Rundeseire</span><strong>{roundWins}</strong></article><article><span>Topp 10 %-runder</span><strong>{top10}</strong></article><article><span>Kapteinsandel</span><strong>{captainShare==null?"—":`${pts(captainShare)} %`}</strong></article><article><span>Beste Bonus/Event Week</span><strong>{bestSpecial?`${special(bestSpecial)} · ${pts(bestSpecial.round_points)} p`:"—"}</strong></article></div></section>
