...
export default function Page(){
 ...
 async function run(){setBusy(true);try{
  ...
  for(const r of roster.rows){
   ...
   if(!s){let best:any=null,bs=0;for(const x of curPlayers){const sc=score(name,x.name);if(sc>bs){bs=sc;best=x}}if(bs>=800)s=best}
   let pr=priceMap.get(norm(name));if(!pr){const m=match(name,p25src as any[]);if(m&&m.score>=800)pr=m.row}
   const pos=String(r.position||s?.position||"").toUpperCase();
   const validPos=POS.includes(pos)?pos:"";
   let old=pr?Number(pr[2])/1e6:null;
   let rawEst:number,pricingClass:string,confidence:string,reason:string,rs=0;
   ...
   raw.push({id:r.personId||`${team}-${name}`,name,team,position:validPos||"—",positionSource:r.positionSource||"HockeyLive",games:s?.games??null,ppg:s?.ppg??null,old,rawEst,repricingScore:rs,pricingClass,confidence,reason,hasStats:Boolean(s),hasOldPrice:Boolean(pr)});
  }
  ...
  const provisional=out.filter((x:any)=>x.pricingClass==="Provisorisk").length,missingPos=out.filter((x:any)=>x.position==="—").length,modelled=out.filter((x:any)=>x.pricingClass==="Modell").length,statsModelled=out.filter((x:any)=>x.pricingClass==="Statsmodell").length,carried=out.filter((x:any)=>x.pricingClass==="Videreført").length,epEnriched=out.filter((x:any)=>x.positionSource==="eliteprospects").length,manualEnriched=out.filter((x:any)=>x.positionSource==="manual").length;
  setRows(out);setStats({...cal.stats,n:out.length,modelled,statsModelled,carried,provisional,missingPos,epEnriched,manualEnriched,up3:out.filter((x:any)=>x.delta>=3).length,down3:out.filter((x:any)=>x.delta<=-3).length});
  setMsg(`Ferdig · ${out.length} spillere priset · ${modelled} full modell · ${statsModelled} statsmodell · ${provisional} provisoriske · ${epEnriched} EP-posisjoner · ${manualEnriched} manuell`)
 }catch(e:any){setMsg(e.message||"Feil")}finally{setBusy(false)}}
 return <main className="fantasy-shell"><section className="fantasy-hero"><div><p className="fantasy-kicker">STANG INN · PRISMODELL V4.1</p><h1>Markedskalibrert 2026/27-pris</h1><p className="fantasy-lead">Renset 2026/27-roster er fasit. HockeyLive bestemmer hvem som er med. Manglende posisjoner fylles fra lokal, verifisert EliteProspects-cache eller manuell overstyring.</p></div><a className="pill" href="/fantasy">← Fantasy-sentralen</a></section><section className="fantasy-card"><button disabled={busy} onClick={run}>{busy?"Beregner …":"Generer v4.1-priser for 2026/27-roster"}</button> <span>{msg}</span>{stats&&<><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(155px,1fr))",gap:10,margin:"18px 0"}}>{[["Spillere",stats.n],["Full modell",stats.modelled],["Statsmodell",stats.statsModelled],["Videreført",stats.carried],["Provisorisk",stats.provisional],["Mangler posisjon",stats.missingPos],["EP-posisjon",stats.epEnriched],["Manuell posisjon",stats.manualEnriched],["25/26 ankersnitt",`${stats.oldAvg.toFixed(2)}m`],["Rått 26/27 snitt",`${stats.rawAvg.toFixed(2)}m`],["Kalibrert snitt",`${stats.calibratedAvg.toFixed(2)}m`],["Markedsskala",`${(stats.scale*100).toFixed(1)}%`],["Lagbudsjett",`${TEAM_BUDGET_M}m`]].map(([a,b]:any)=><div key={a} style={{padding:14,borderRadius:12,background:'#f5f7fa'}}><b>{a}</b><div style={{fontSize:24,fontWeight:900}}>{b}</div></div>)}</div><div style={{padding:12,borderRadius:12,background:'#fff4df',border:'1px solid #e6c989',marginBottom:18}}><b>Datakilder:</b> HockeyLive er fasit for 2026/27-spillerpoolen. Lokal EliteProspects-cache og manuelle verifiseringer brukes kun på manglende posisjon. «Statsmodell» betyr at spilleren har validert 2025/26-statistikk, men mangler historisk fantasypris. Små kampprøver trekkes nå mot posisjonsmedianen. «Provisorisk» er reelle nye/tynne datapunkter som fortsatt trenger ekstern historikk.</div><div style={{overflowX:'auto'}}><table style={{width:'100%',minWidth:1520,borderCollapse:'collapse',fontSize:12}}><thead><tr>{['Spiller','Pos','Pos-kilde','26/27 lag','K','FP/K','Prisanker','Råmodell','V4.1 pris','Endring','Klasse','Tillit','Reprising','Grunnlag'].map(h=><th key={h} style={{padding:8,textAlign:'left',background:'#eef3f8'}}>{h}</th>)}</tr></thead><tbody>{rows.map((x:any)=><tr key={x.id}><td style={{padding:8,fontWeight:700}}>{x.name}</td><td>{x.position}</td><td>{sourceLabel(x.positionSource)}</td><td>{x.team}</td><td>{x.games??'—'}</td><td>{x.ppg??'—'}</td><td>{x.old.toFixed(1)}m</td><td>{x.rawEst.toFixed(2)}m</td><td style={{fontWeight:900}}>{x.est.toFixed(1)}m</td><td style={{fontWeight:800}}>{x.delta>0?'+':''}{x.delta.toFixed(1)}m</td><td>{x.pricingClass}</td><td>{x.confidence}</td><td>{x.repricingScore}%</td><td>{x.reason}</td></tr>)}</tbody></table></div></>}</section></main>}