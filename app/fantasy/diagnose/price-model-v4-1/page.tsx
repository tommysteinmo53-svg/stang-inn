...
export default function Page(){
 ...
 async function run(){setBusy(true);try{
  ...
  const [a,b,rr]=await Promise.all([
   ...
   fetch('/api/fantasy-roster-2026',{cache:'no-store'})
  ...
  for(const r of roster.rows){
   ...
   raw.push({id:r.personId||`${team}-${name}`,name,team,position:validPos||"—",games:s?.games??null,ppg:s?.ppg??null,old,rawEst,repricingScore:rs,pricingClass,confidence,reason,hasStats:Boolean(s),hasOldPrice:Boolean(pr)});
  }
  ...
  const provisional=out.filter((x:any)=>x.pricingClass==="Provisorisk").length,missingPos=out.filter((x:any)=>x.position==="—").length,modelled=out.filter((x:any)=>x.pricingClass==="Modell").length,carried=out.filter((x:any)=>x.pricingClass==="Videreført").length;
  setRows(out);setStats({...cal.stats,n:out.length,modelled,carried,provisional,missingPos,up3:out.filter((x:any)=>x.delta>=3).length,down3:out.filter((x:any)=>x.delta<=-3).length});
  setMsg(`Ferdig · ${out.length} av ${roster.confirmedPlayers||roster.rows.length} spillere priset · ${modelled} full modell · ${provisional} provisoriske`)
 }catch(e:any){setMsg(e.message||"Feil")}finally{setBusy(false)}}
 return <main className="fantasy-shell"><section className="fantasy-hero"><div><p className="fantasy-kicker">STANG INN · PRISMODELL V4.1</p><h1>Markedskalibrert 2026/27-pris</h1><p className="fantasy-lead">Renset 2026/27-roster er fasit. Etablerte spillere prises fra validert historikk, mens nye spillere får en eksplisitt provisorisk startpris til posisjon og ekstern historikk er beriket.</p></div><a className="pill" href="/fantasy">← Fantasy-sentralen</a></section><section className="fantasy-card"><button disabled={busy} onClick={run}>{busy?"Beregner …":"Generer v4.1-priser for 2026/27-roster"}</button> <span>{msg}</span>{stats&&<><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(155px,1fr))",gap:10,margin:"18px 0"}}>{[["Spillere",stats.n],["Full modell",stats.modelled],["Videreført",stats.carried],["Provisorisk",stats.provisional],["Mangler posisjon",stats.missingPos],["25/26 ankersnitt",`${stats.oldAvg.toFixed(2)}m`],["Rått 26/27 snitt",`${stats.rawAvg.toFixed(2)}m`],["Kalibrert snitt",`${stats.calibratedAvg.toFixed(2)}m`],["Markedsskala",`${(stats.scale*100).toFixed(1)}%`],["Lagbudsjett",`${TEAM_BUDGET_M}m`]].map(([a,b]:any)=><div key={a} style={{padding:14,borderRadius:12,background:'#f5f7fa'}}><b>{a}</b><div style={{fontSize:24,fontWeight:900}}>{b}</div></div>)}</div><div style={{padding:12,borderRadius:12,background:'#fff4df',border:'1px solid #e6c989',marginBottom:18}}><b>Viktig:</b> «Provisorisk» betyr at spilleren er ekte 2026/27-roster, men mangler validert EHL-historikk/pris. Disse får foreløpig posisjonsmedian, eller markedsmedian hvis posisjon mangler. De skal senere berikes med tidligere liga/klubb før prisene låses.</div><div style={{overflowX:'auto'}}><table style={{width:'100%',minWidth:1450,borderCollapse:'collapse',fontSize:12}}><thead><tr>{['Spiller','Pos','26/27 lag','K','FP/K','25/26 anker','Råmodell','V4.1 pris','Endring','Klasse','Tillit','Reprising','Grunnlag'].map(h=><th key={h} style={{padding:8,textAlign:'left',background:'#eef3f8'}}>{h}</th>)}</tr></thead><tbody>{rows.map((x:any)=><tr key={x.id}><td style={{padding:8,fontWeight:700}}>{x.name}</td><td>{x.position}</td><td>{x.team}</td><td>{x.games??'—'}</td><td>{x.ppg??'—'}</td><td>{x.old.toFixed(1)}m</td><td>{x.rawEst.toFixed(2)}m</td><td style={{fontWeight:900}}>{x.est.toFixed(1)}m</td><td style={{fontWeight:800}}>{x.delta>0?'+':''}{x.delta.toFixed(1)}m</td><td>{x.pricingClass}</td><td>{x.confidence}</td><td>{x.repricingScore}%</td><td>{x.reason}</td></tr>)}</tbody></table></div></>}</section></main>}
