...
export default function RoundPointsView({rows,title="Rundehistorikk",intro="Åpne tidligere fantasy-runder og se nøyaktig laget som ble låst ved deadline. Historiske lag kommer alltid fra autoritative snapshots.",emptyMessage="Ingen låste fantasy-runder ennå.",testMode=false}:{rows:RoundDetail[];title?:string;intro?:string;emptyMessage?:string;testMode?:boolean}){
 ...
 const renderLine=(lineNo:number)=>{if(!current)return null;const players=current.players.filter(p=>p.line_no===lineNo).sort((a,b)=>order[posGroup(a.player_position)]-order[posGroup(b.player_position)]||a.player_name.localeCompare(b.player_name,"nb"));const lineMultiplier=current.line2_multiplier_override!=null&&lineNo===2?current.line2_multiplier_override:(lineNo===2?0.5:1);return <section className="round-line-block"><div className="round-line-head"><div><span>REKKE {lineNo}</span><strong>{pts(lineMultiplier)}× uttelling</strong></div>{lineNo===2&&current.booster_type==="line_boost"&&<em>🔥 REKKEBOOST</em>}</div><div className="round-player-list">{players.map(p=><div className="round-player-row" key={p.player_id}><span className={`team-pos team-pos-${posGroup(p.player_position).toLowerCase()}`}>{posGroup(p.player_position)}</span><div className="round-player-main"><strong><a href={`/fantasy/players/${p.player_id}`} className="round-player-link">{p.player_name}</a>{roleText(p,current)&&<em className="round-role">{roleText(p,current)}</em>}</strong><small>{p.player_team} · {p.player_position} · {pts(p.player_price)}m{current.is_scored?` · ${p.games_played??0} ${(p.games_played??0)===1?"kamp":"kamper"}${p.played===false?" · spilte ikke":""}`:""}</small></div><div className="round-player-calc">{current.is_scored?<><small>{pts(p.raw_points)} rå · rekke ×{pts(p.line_multiplier??lineMultiplier)}{(p.role_multiplier??1)>1?` · rolle ×${pts(p.role_multiplier)}`:""}</small><strong>{pts(p.player_total_points)} p</strong></>:<><small>Snapshot ved deadline</small><strong>—</strong></>}</div></div>)}</div></section>};
