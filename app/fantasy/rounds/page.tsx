...
type Game={id:string;fantasy_round_id:string|null;starts_at:string|null;home_team:string;away_team:string};
...
export default function FantasyRoundsPage(){
 ...
 useEffect(()=>{(async()=>{try{const sb=getSupabaseBrowserClient();if(!sb){setAuth(false);return}const{data:s}=await sb.auth.getSession();if(!s.session){setAuth(false);return}setAuth(true);const[{data:r,error},{data:g,error:ge}]=await Promise.all([sb.from("fantasy_rounds").select("id,round_no,name,starts_at,deadline_at,ends_at,status").eq("season",SEASON).lt("round_no",9000).order("round_no"),sb.from("fantasy_games").select("id,fantasy_round_id,starts_at,home_team,away_team").eq("season",SEASON).not("fantasy_round_id","is",null).order("starts_at")]);if(error)throw error;setRounds((r||[]) as Round[]);if(!ge)setGames((g||[]) as Game[])}catch(e:any){setMessage(`Kunne ikke hente fantasy-runder: ${e?.message||e}`)}})()},[]);
 const gameMap=useMemo(()=>{const m=new Map<string,Game[]>();for(const g of games){if(!g.fantasy_round_id)continue;const a=m.get(g.fantasy_round_id)||[];a.push(g);m.set(g.fantasy_round_id,a)}return m},[games]);
 ...
 return <main className="fantasy-shell rounds-shell">
  ...
  <section className="team-metric-grid"><article><span>Fantasy-runder</span><strong>{rounds.length}</strong></article><article><span>Kamper</span><strong>{totalGames||225}</strong></article><article><span>Ferdige runder</span><strong>{finished}</strong></article><article><span>Neste runde</span><strong>{next?`R${next.round_no}`:"—"}</strong></article><article><span>Neste deadline</span><strong className="rounds-deadline-metric">{next?fmt(next.deadline_at):"—"}</strong></article></section>
  {message&&<p className="team-error">{message}</p>}
  <section className="team-panel rounds-panel"><div className="leaderboard-section-head"><div><p className="eyebrow">KALENDER</p><h2>Alle runder</h2></div><span className="team-muted">Klikk på en runde for å se kampene</span></div>
   <div className="rounds-list">{rounds.map(r=>{const rg=gameMap.get(r.id)||[],open=selected===r.id,isNext=next?.id===r.id;return <article key={r.id} className={`round-card ${isNext?"next":""}`}><button className="round-card-main" onClick={()=>setSelected(open?null:r.id)}><div className="round-number"><small>RUNDE</small><strong>{r.round_no}</strong></div><div className="round-summary"><strong>{r.name||`Fantasy-runde ${r.round_no}`}{isNext&&<em>NESTE</em>}</strong><small>Deadline {fmt(r.deadline_at)}</small></div><div className="round-stat"><small>Kamper</small><b>{rg.length||"—"}</b></div><div className="round-stat"><small>Status</small><b className={`round-status ${r.status}`}>{statusLabel(r.status)}</b></div><span className="leaderboard-chevron">{open?"▾":"▸"}</span></button>{open&&<div className="round-games">{rg.length===0?<p className="team-muted">Kampdetaljer er ikke tilgjengelige i denne visningen ennå.</p>:rg.map(g=><div key={g.id} className="round-game"><span>{fmt(g.starts_at)}</span><strong>{g.home_team}</strong><b>–</b><strong>{g.away_team}</strong></div>)}</div>}</article>})}</div>
