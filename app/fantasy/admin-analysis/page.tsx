...
type SortKey="xfp_next_game"|"xfp_next3"|"value_next3"|"price"|"player_name";
...
export default function AdminAnalysisPage(){
  ...
  const[sort,setSort]=useState<SortKey>("xfp_next_game"),[teamFilter,setTeamFilter]=useState("ALL"),[posFilter,setPosFilter]=useState("ALL"),[q,setQ]=useState("");
  ...
  const shown=useMemo(()=>xfpRows.filter(r=>(teamFilter==="ALL"||r.team===teamFilter)&&(posFilter==="ALL"||pos(r.player_position)===posFilter)&&(!q||`${r.player_name} ${r.team}`.toLowerCase().includes(q.toLowerCase()))).sort((a,b)=>sort==="player_name"?a.player_name.localeCompare(b.player_name,"nb"):Number(b[sort])-Number(a[sort])),[xfpRows,teamFilter,posFilter,q,sort]);
  ...
  return <main className="fantasy-shell">
    ...
    <section className="fantasy-card" style={{marginTop:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginTop:16}}>
        {[
          ["Sesongprestasjon",seasonWeight,setSeasonWeight],
          ["Form siste 5",formWeight,setFormWeight],
          ["Hjemme/borte",venueWeight,setVenueWeight],
          ["Motstander",opponentWeight,setOpponentWeight],
        ].map(([label,value,setter]:any)=><label key={label} className="team-info-card" style={{display:"grid",gap:8}}><span>{label}</span><strong style={{fontSize:22}}>{value}%</strong><input type="range" min="0" max="100" step="5" value={value} onChange={e=>setter(Number(e.target.value))}/></label>)}
      </div>
    ...
    <section className="fantasy-card" style={{marginTop:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"minmax(180px,1fr) repeat(3,minmax(130px,auto))",gap:10,marginTop:14}}>
        ...
        <select className="team-line-select" value={sort} onChange={e=>setSort(e.target.value as SortKey)}><option value="xfp_next_game">xFP neste kamp</option><option value="xfp_next3">xFP neste 3</option><option value="value_next3">Beste verdi</option><option value="price">Pris</option><option value="player_name">Navn A–Å</option></select>
      </div>

      <div style={{overflowX:"auto",marginTop:14}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:900}}><thead><tr><th style={{textAlign:"left",padding:10}}>Spiller</th><th>Pris</th><th>Sesong</th><th>Form</th><th>Neste</th><th>xFP kamp</th><th>xFP 3</th><th>Verdi</th><th>Data</th></tr></thead><tbody>{shown.map(r=><tr key={r.player_id} style={{borderTop:"1px solid rgba(255,255,255,.08)"}}><td style={{padding:10}}><strong>{r.player_name}</strong><br/><small>{r.team} · {pos(r.player_position)}</small></td><td style={{textAlign:"center"}}>{n(r.price,1)}m</td><td style={{textAlign:"center"}}>{n(r.season_ppg)}</td><td style={{textAlign:"center"}}>{n(r.form_ppg)}</td><td style={{textAlign:"center"}}>{r.opponent?<><strong>{r.is_home?"vs":"@"} {r.opponent}</strong><br/><small>{r.next3_games} kamper / neste 3</small></>:"—"}</td><td style={{textAlign:"center"}}><strong>{n(r.xfp_next_game)}</strong></td><td style={{textAlign:"center"}}><strong>{n(r.xfp_next3)}</strong></td><td style={{textAlign:"center"}}>{n(r.value_next3,3)}</td><td style={{textAlign:"center"}}>{r.data_confidence==="high"?"🟢 Høy":r.data_confidence==="medium"?"🟡 Middels":"🔴 Lav"}<br/><small>{r.games_scored} kamper</small></td></tr>)}{!shown.length&&!xfpBusy&&<tr><td colSpan={9} style={{padding:20,textAlign:"center"}}>Ingen spillere matcher filtrene.</td></tr>}</tbody></table></div>
    </section>

    <section className="fantasy-grid" style={{marginTop:18}}>
      {modules.filter(m=>m.key!=="expected-points").map(m=><article className="fantasy-card" key={m.key}><p className="eyebrow">ADMIN ONLY</p><h2>{m.label}</h2><p className="card-copy">{details[m.key]}</p><p><strong>Status:</strong> bygges i neste fase.</p></article>)}
    </section>
