...
type Row={
  ...
  adjusted_xfp_next_game:number;adjusted_xfp_next_round:number;adjusted_xfp_next3_rounds:number;
...
export default function XfpHorizonsPage(){
  ...
  return <main className="fantasy-shell horizon-shell">
    ...
    <section className="horizon-panel">
      ...
      <div className="horizon-table-wrap"><table><thead><tr><th>Spiller</th><th>Lag</th><th>Pos</th><th>Pris</th><th>Base xFP kamp</th><th>Justert xFP kamp</th><th>xFP neste runde</th><th>Kamper</th><th>xFP 3 runder</th><th>Kamper</th><th>Availability</th><th>Data</th></tr></thead><tbody>
        {shown.map(r=><tr key={r.player_id}><td><strong>{r.player_name}</strong></td><td>{r.team}</td><td>{pos(r.player_position)}</td><td>{n(r.price,1)}m</td><td>{n(r.base_xfp_next_game)}</td><td className="xfp-now">{n(r.adjusted_xfp_next_game)}</td><td className="xfp-round">{n(r.adjusted_xfp_next_round)}</td><td>{r.next_round_games}</td><td className="xfp-three">{n(r.adjusted_xfp_next3_rounds)}</td><td>{r.next3_round_games}</td><td><span className={`availability ${r.availability_status}`}>{r.availability_status}</span><small>{Math.round(r.availability_factor*100)} %</small></td><td>{conf(r.data_confidence)}</td></tr>)}
        {!busy&&!shown.length&&<tr><td colSpan={12} className="empty">Ingen spillere matcher filtrene.</td></tr>}
      </tbody></table></div>
      <p className="horizon-note"><strong>Viktig:</strong> Tallene i «xFP neste runde» og «xFP 3 runder» summerer alle kampene spilleren faktisk har i de kalenderbaserte fantasy-rundene. En spiller kan derfor ha 0, 1 eller flere kamper i en runde.</p>
    </section>
  </main>;
}
