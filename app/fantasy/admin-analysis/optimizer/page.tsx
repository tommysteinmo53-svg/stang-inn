...
type StrategyProposal={key:StrategyKey;label:string;description:string;optimized:TransferPlayer[];changes:TransferChange[];optimized_cost:number;optimized_score:number;xfp_gain:number;proposal_risk_score:number;proposal_risk_label:"Lav"|"Middels"|"Høy";modeled_upside:number;objective_score:number};
type TransferResult={team:{id:string;name:string}|null;status:any;economy:Economy|null;current:TransferPlayer[];optimized:TransferPlayer[];changes:TransferChange[];strategies:StrategyProposal[];current_cost:number;optimized_cost:number;current_score:number;optimized_score:number;xfp_gain:number;transfers_available:number;proposal_risk_score:number;proposal_risk_label:"Lav"|"Middels"|"Høy";message?:string};
...
function StrategyChanges({proposal,available}:{proposal:StrategyProposal;available:number}){
 return <section className={`strategy-detail strategy-${proposal.key}`}>
  <div className="strategy-detail-head"><div><p className="eyebrow">{proposal.label}</p><h3>{proposal.description}</h3></div><div className="strategy-metrics"><span><b className={proposal.xfp_gain>=0?"positive":"negative"}>{proposal.xfp_gain>=0?"+":""}{fmt(proposal.xfp_gain,2)}</b><small>forventet gevinst</small></span><span><b>{proposal.proposal_risk_label}</b><small>{proposal.proposal_risk_score}/100 risiko</small></span><span><b>{fmt(proposal.modeled_upside,2)}</b><small>modellert oppside</small></span></div></div>
  {!proposal.changes.length?<p className="team-message">Denne strategien finner ingen bytter som forbedrer sitt risiko-/oppsidejusterte objektiv.</p>:<div className="transfer-change-grid">{proposal.changes.map((c,i)=>{const outSlot=transferSlot(c.out),inSlot=transferSlot(c.in),role=transferRole(c.in);return <article className="transfer-change-card" key={`${proposal.key}-${c.out.id}-${c.in.id}`}><div className="transfer-change-no">Bytte {i+1}/{Math.max(1,available)} · {c.out.pos==="G"?"Keeper":outSlot===inSlot?inSlot:`${outSlot} → ${inSlot}`}</div><div className="transfer-side out"><span>UT</span><strong>{c.out.name}{playerRole(c.out)}</strong><small>{c.out.team} · {c.out.pos} · {fmt(c.out.price)}m</small><small>Base {fmt(c.out.raw_base_score,2)} → availability {fmt(c.out.base_score,2)} → effektiv {fmt(c.out.score,2)} xFP</small></div><div className="transfer-arrow">→</div><div className="transfer-side in"><span>INN</span><strong>{c.in.name}{playerRole(c.in)}</strong><small>{c.in.team} · {c.in.pos} · {fmt(c.in.price)}m</small><small>Base {fmt(c.in.raw_base_score,2)} → availability {fmt(c.in.base_score,2)} → effektiv {fmt(c.in.score,2)} xFP</small><small>Risiko {c.risk_label} ({c.risk_score}/100) · datatillit {confidenceLabel(c.in.data_confidence)} · {availabilityLabel(c.in.availability_status)}</small>{role&&<small>{role}rollen følger plassen i dette forslaget.</small>}</div><footer><b className={c.xfp_gain>=0?"positive":"negative"}>{c.xfp_gain>=0?"+":""}{fmt(c.xfp_gain,2)} direkte effektiv xFP</b><span>{c.price_change>=0?"+":""}{fmt(c.price_change)}m</span></footer></article>})}</div>}
 </section>;
}
...
export default function OptimizerPage(){
 ...
 return <main className="fantasy-shell xfp-command-center">
  ...
  <section className="xfp-panel transfer-optimizer-panel">
   <div className="xfp-panel-head"><div><p className="eyebrow">DITT LAG · BYTTEOPTIMALISERING</p><h2>Balansert · konservativ · offensiv</h2><p>Tre reelt modellberegnede strategier bruker samme Fantasy-regler og samme availability-justerte forventning, men ulik toleranse for risiko og modellert oppside.</p></div><button className="xfp-primary" disabled={transferBusy} onClick={()=>runTransfers()}>{transferBusy?"Analyserer …":"🔄 Sammenlign strategier"}</button></div>
   {transferResult&&<div className="optimizer-summary transfer-summary"><div><span>Ledige bytter</span><strong>{transferResult.transfers_available}</strong><small>{Number(transferResult.status?.transfers_used||0)} brukt i runden</small></div><div><span>Nåværende effektiv xFP</span><strong>{fmt(transferResult.current_score,2)}</strong><small>{horizon==="next_game"?"neste kamp":"neste 3 kamper"} · etter availability/rekkefaktor/C/VC</small></div><div><span>Strategier</span><strong>{transferResult.strategies?.length||0}/3</strong><small>samme regler · ulike objektiver</small></div><div><span>Modell</span><strong>MP-10.4</strong><small>forventning · risiko · oppside</small></div></div>}{transferMessage&&<p className="team-message">{transferMessage}</p>}
   ...
   {!!transferResult?.strategies?.length&&<><div className="strategy-grid">{transferResult.strategies.map(s=><article className={`strategy-card strategy-${s.key}`} key={s.key}><p className="eyebrow">{s.label}</p><strong className={s.xfp_gain>=0?"positive":"negative"}>{s.xfp_gain>=0?"+":""}{fmt(s.xfp_gain,2)} xFP</strong><span>{s.proposal_risk_label} risiko · {s.proposal_risk_score}/100</span><small>{s.changes.length} bytte{s.changes.length===1?"":"r"} · modellert oppside {fmt(s.modeled_upside,2)}</small><p>{s.description}</p></article>)}</div><p className="team-message">Tallene holdes adskilt: <b>base-xFP</b> er modellens rå prognose; <b>availability-justert xFP</b> bruker 100/85/60/0 %; <b>effektiv Fantasy-xFP</b> inkluderer deretter rekke 1/2 og C/VC; <b>risikoscore</b> er kun usikkerhet og endrer ikke Fantasy-scoring.</p>{transferResult.strategies.map(s=><StrategyChanges key={s.key} proposal={s} available={transferResult.transfers_available}/>)}</>}
  </section>
 </main>;
}
