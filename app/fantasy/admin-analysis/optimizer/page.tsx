...
export default function OptimizerPage(){
 ...
 return <main className="fantasy-shell xfp-command-center">
  ...
  <section className="xfp-panel transfer-optimizer-panel">
   ...
   {!!transferResult?.changes?.length&&<><div className="transfer-roster-title"><h3>Optimale bytter</h3><span>{transferResult.changes.length}/{transferResult.transfers_available} bytter</span></div><div className="transfer-change-grid">{transferResult.changes.map((c,i)=><article className="transfer-change-card" key={`${c.out.id}-${c.in.id}`}><div className="transfer-change-no">Bytte {i+1}{c.line_no?` · Rekke ${c.line_no}`:""}</div><div className="transfer-side out"><span>UT</span><strong>{c.out.name}</strong><small>{c.out.team} · {c.out.pos} · {fmt(c.out.price)}m · {fmt(c.out.score,2)} xFP</small></div><div className="transfer-arrow">→</div><div className="transfer-side in"><span>INN</span><strong>{c.in.name}</strong><small>{c.in.team} · {c.in.pos} · {fmt(c.in.price)}m · {fmt(c.in.score,2)} xFP</small></div><footer><b className={c.xfp_gain>=0?"positive":"negative"}>{c.xfp_gain>=0?"+":""}{fmt(c.xfp_gain,2)} xFP</b><span>{c.price_change>=0?"+":""}{fmt(c.price_change)}m</span></footer></article>)}</div></>}
