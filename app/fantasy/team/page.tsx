...
function normalizeTeamName(value:string){return value.trim().replace(/\s+/g," ")}
function teamNameError(value:string){
 const normalized=normalizeTeamName(value),length=Array.from(normalized).length;
 if(!normalized)return"Du må velge et lagnavn";
 if(/[\u0000-\u001F\u007F]/.test(value))return"Lagnavnet inneholder ugyldige tegn";
 if(length<3||length>40)return"Lagnavnet må være mellom 3 og 40 tegn";
 if(PLACEHOLDER_NAMES.has(normalized.toLocaleLowerCase("nb-NO")))return"Velg et personlig lagnavn – «Mitt lag» kan ikke brukes";
 if(!/[\p{L}\p{N}]/u.test(normalized))return"Lagnavnet må inneholde minst én bokstav eller ett tall";
 return"";
}
function buildLine1(ids:string[],players:Player[],preferred:string[]=[]){
 const chosen=ids.map(id=>players.find(p=>p.id===id)).filter(Boolean)as Player[],out:string[]=[];
 for(const[g,n]of[["F",3],["D",2],["G",1]]as const){
  const candidates=chosen.filter(p=>group(p)===g),pref=candidates.filter(p=>preferred.includes(p.id));
...
export default function FantasyTeamPage(){
 ...
 const[teamName,setTeamName]=useState(""),[savedTeamName,setSavedTeamName]=useState(""),[nameNeedsCompletion,setNameNeedsCompletion]=useState(false);
 const[msg,setMsg]=useState("Laster spillerpool …"),[busy,setBusy]=useState(false),[nameBusy,setNameBusy]=useState(false),[filter,setFilter]=useState<"ALL"|"F"|Pos>("ALL"),[clubFilter,setClubFilter]=useState("ALL"),[q,setQ]=useState("");
 ...
 const[seasonStarted,setSeasonStarted]=useState(false),[transferStatus,setTransferStatus]=useState<TransferStatus|null>(null),[rounds,setRounds]=useState<Round[]>([]),[roundGames,setRoundGames]=useState<Game[]>([]);
 ...
 useEffect(()=>{(async()=>{try{
  ...
  if(t){
   setTeamId(t.id);
   const storedName=String(t.name||"");
   const invalidStoredName=Boolean(teamNameError(storedName));
   setSavedTeamName(invalidStoredName?"":normalizeTeamName(storedName));
   setTeamName(invalidStoredName?"":normalizeTeamName(storedName));
   setNameNeedsCompletion(invalidStoredName);
   const{data:tp}=await s.from("fantasy_user_team_players").select("player_id,is_captain,is_vice_captain,line_no").eq("team_id",t.id);
   const roster=(tp||[]).map((x:any)=>x.player_id);setSelected(roster);setInitialRoster(roster);
   setCaptain((tp||[]).find((x:any)=>x.is_captain)?.player_id||null);setViceCaptain((tp||[]).find((x:any)=>x.is_vice_captain)?.player_id||null);
   const stored=(tp||[]).filter((x:any)=>Number(x.line_no)===1).map((x:any)=>x.player_id);
   setLine1(validLine1(stored,roster.map(id=>pool.find(x=>x.id===id)).filter(Boolean)as Player[])?stored:buildLine1(roster,pool,stored));
 ...
 function toggle(p:Player){
  ...
  if(p.position==="D"&&counts.D>=4){setMsg("Du har allerede 4 backer");return}
  if(p.position==="G"&&counts.G>=2){setMsg("Du har allerede 2 keepere");return}
  if((clubCounts.get(p.team)||0)>=rules.max_players_per_club){setMsg(`Maks ${rules.max_players_per_club} spillere fra ${p.team}`);return}
  if(total+p.price>BUDGET){setMsg(`Budsjettet overskrides med ${(total+p.price-BUDGET).toFixed(1)}m`);return}
 ...
 async function saveTeamName(){
  if(!teamId||nameError)return;
  setNameBusy(true);
  try{
   const s=getSupabaseBrowserClient();if(!s)throw new Error("Supabase er ikke tilgjengelig");
   const{data,error}=await s.rpc("rename_fantasy_team_v1",{p_season:SEASON,p_name:teamName});if(error)throw error;
   const normalized=String(data||normalizeTeamName(teamName));setTeamName(normalized);setSavedTeamName(normalized);setNameNeedsCompletion(false);
   setMsg("Lagnavnet er lagret ✓ · dette brukte ingen bytter");
  }catch(e:any){setMsg(`Lagnavn ble ikke lagret: ${e.message||e}`)}finally{setNameBusy(false)}
 }
 ...
 async function save(){
  ...
  try{
   ...
   if(seasonStarted&&teamId){const{error}=await s.rpc("apply_fantasy_transfers_v1",{p_season:SEASON,p_name:normalizedName,p_player_ids:selected,p_captain:captain,p_vice_captain:viceCaptain});if(error)throw error}
   else{const{data,error}=await s.rpc("save_fantasy_team_v3",{p_season:SEASON,p_name:normalizedName,p_player_ids:selected,p_captain:captain,p_vice_captain:viceCaptain});if(error)throw error;savedTeamId=data;setTeamId(data)}
   const{error:le}=await s.rpc("set_fantasy_lineup_v1",{p_season:SEASON,p_line1_player_ids:line1});if(le)throw le;
   setInitialRoster([...selected]);if(savedTeamId)setTeamId(savedTeamId);setTeamName(normalizedName);setSavedTeamName(normalizedName);setNameNeedsCompletion(false);
 ...
 return <main className="fantasy-shell team-builder-shell">
  ...
  {nameNeedsCompletion&&<section className="team-name-alert"><strong>Gi laget ditt et navn</strong><span>Det eksisterende laget ditt er bevart, men «Mitt lag» kan ikke brukes som ferdig lagnavn. Velg et navn før du lagrer andre endringer.</span></section>}
