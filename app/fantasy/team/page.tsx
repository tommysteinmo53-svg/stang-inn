...
export default function FantasyTeamPage(){
 const[players,setPlayers]=useState<Player[]>([]),[selected,setSelected]=useState<string[]>([]),[teamName,setTeamName]=useState("Mitt lag"),[msg,setMsg]=useState("Laster spillerpool …"),[busy,setBusy]=useState(false),[filter,setFilter]=useState<"ALL"|"F"|"C"|"W"|"D"|"G">("ALL"),[q,setQ]=useState("");
 ...
 const counts=useMemo(()=>({F:chosen.filter(p=>p.position==="C"||p.position==="W").length,C:chosen.filter(p=>p.position==="C").length,W:chosen.filter(p=>p.position==="W").length,D:chosen.filter(p=>p.position==="D").length,G:chosen.filter(p=>p.position==="G").length}),[chosen]);
 ...
 const valid=selected.length===12&&left>=0&&counts.F===6&&counts.D===4&&counts.G===2&&!clubOverflow&&!!captain&&!!viceCaptain&&captain!==viceCaptain;
 const visible=players.filter(p=>{
  const positionOk=filter==="ALL"||(filter==="F"?(p.position==="C"||p.position==="W"):p.position===filter);
  return positionOk&&(!q||`${p.name} ${p.team}`.toLowerCase().includes(q.toLowerCase()));
 });
 function toggle(p:Player){
  if(selected.includes(p.id)){setSelected(selected.filter(x=>x!==p.id));if(captain===p.id)setCaptain(null);if(viceCaptain===p.id)setViceCaptain(null);return}
  if(selected.length>=12){setMsg("Laget har allerede 12 spillere");return}
  const isForward=p.position==="C"||p.position==="W";
  if(isForward&&counts.F>=6){setMsg("Du har allerede 6 forwards");return}
  ...
  setSelected([...selected,p.id]);setMsg(`${p.name} lagt til`)
 }
 ...
 async function save(){if(!valid)return;setBusy(true);try{const s=getSupabaseBrowserClient();if(!s)throw new Error("Supabase er ikke tilgjengelig");const{data,error}=await s.rpc("save_fantasy_team_v3",{p_season:"2026/27",p_name:teamName,p_player_ids:selected,p_captain:captain,p_vice_captain:viceCaptain});if(error)throw error;setMsg(`Lag lagret ✓ · ID ${data}`)}catch(e:any){setMsg(`Lagring stoppet: ${e.message||e}`)}finally{setBusy(false)}}
 return <main className="fantasy-shell"><section className="fantasy-hero"><div><p className="fantasy-kicker">STANG INN · FANTASY 2026/27</p><h1>Bygg laget ditt</h1><p className="fantasy-lead">12 spillere · 6F · 4D · 2G · C og W kan brukes om hverandre · maks 100,0m · maks {rules.max_players_per_club} fra samme klubb.</p></div></section>
