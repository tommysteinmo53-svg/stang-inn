...
type Status={teamName:string;players:number;captain:boolean;vice:boolean;teamCost:number;roundNo:number|null;deadline:string|null;used:number;remaining:number;limit:number};
...
export default function TeamLaunchStatus(){
 ...
 return <section className={`team-launch-status ${ready?"ready":"needs-work"}`}>
  ...
  {message?<p>{message}</p>:data&&<div className="team-launch-checks"><span className={data.players===12?"ok":""}>12 spillere: {data.players}/12</span><span className={data.captain&&data.vice?"ok":""}>C ×2 og VC ×1,5</span><span className={data.teamCost<=110?"ok":""}>Lagverdi {data.teamCost.toFixed(1)}m / 110m</span><span className="ok">Bytter {data.used}/{data.limit} · {data.remaining} igjen</span></div>}
