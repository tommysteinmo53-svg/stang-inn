...
import { EHL_ACTIVE_TEAMS } from "../../lib/ehl-season";
...
const defaultOrder = EHL_ACTIVE_TEAMS;
const teamCount = defaultOrder.length;
...
export default function TableTipsPage() {
  ...
  async function load() {
    ...
    const [p, t, settings, s, d, sc] = await Promise.all([
      supabase.from("players").select("id,display_name").order("display_name"), supabase.from("table_tips").select("player_id,team,position").order("position"), supabase.from("app_settings").select("value").eq("key", "table_tips").maybeSingle(), supabase.from("ehl_standings").select("team,position,played,points,synced_at").eq("season", "2026/27").eq("active", true).order("position"), supabase.from("table_tip_deviation").select("player_id,team,predicted_position,actual_position,deviation").order("predicted_position"), supabase.from("table_tip_scores").select("player_id,display_name,compared_teams,total_deviation,worst_deviation,exact_positions").order("total_deviation")]);
    setPlayers((p.data || []) as Player[]); const tableRows = (t.data || []) as TableTip[]; setRows(tableRows); setStandings((s.data || []) as EhlStanding[]); setDeviations((d.data || []) as Deviation[]); setScores(((sc.data || []) as TableScore[]).filter(x => x.compared_teams === teamCount));
    const own = tableRows.filter(r => r.player_id === uid).sort((a,b) => a.position - b.position); if (own.length === teamCount) setOrder(own.map(r => r.team));
    const value = settings.data?.value as { deadline?: string | null } | undefined; const nextDeadline = value?.deadline || null; setDeadline(nextDeadline); setLocked(!nextDeadline || !Number.isFinite(Date.parse(nextDeadline)) || Date.now() >= Date.parse(nextDeadline)); setLoading(false);
  }
  useEffect(() => { load(); }, []);
  ...
  return <main className="appShell tableTipsPage"><header className="topbar"><a href="/" className="brand brandButton" style={{textDecoration:"none"}}><div className="brandMark">🏒</div><div><p className="eyebrow">EHL 2026/27</p><h1>Tabelltips</h1></div></a><a href="/" className="textButton">← Hjem</a></header>
    <section className="pageStack tableTipsStack" style={{marginTop:24}}>
      <SeasonChangeNotice />
      <article className={`heroCard tableTipsHero ${locked?"locked":"open"}`}><div><p className="eyebrow">Sesongkonkurranse</p><h2>{locked ? "Tabelltipset er låst" : ownSavedCount===teamCount ? "Tabelltipset ditt er klart ✓" : "Sett din EHL-tabell"}</h2><p className="muted">Ranger alle {teamCount} lag. Når sesongen starter vinner lavest samlet plasseringsavvik.</p></div><div className="countdown"><strong>{locked?"🔒":`${ownSavedCount}/${teamCount}`}</strong><span>{locked?"fristen er passert":"lag lagret"}</span></div></article>
      <div className="tableTipsSummary"><article className="miniCard"><span>Frist</span><strong>{deadline ? formatShortDate(deadline) : "Ikke satt"}</strong><small>{locked?"Låst":"Kan endres frem til fristen"}</small></article><article className="miniCard"><span>Din status</span><strong>{ownSavedCount===teamCount?"Levert ✓":"Ikke komplett"}</strong><small>{ownSavedCount}/{teamCount} plasseringer lagret</small></article><article className="miniCard"><span>{seasonStarted?"Din score":"EHL-status"}</span><strong>{seasonStarted&&ownScore?ownScore.total_deviation:seasonStarted?"–":"Før start"}</strong><small>{seasonStarted&&ownScore?`${ownScore.exact_positions} eksakte`:"Avvik aktiveres etter første kamp"}</small></article></div>
      ...
      <section className="contentGrid tableTipsMainGrid">
        <article className="panel tableTipEditor"><div className="panelHeading"><div><p className="eyebrow">Mitt tips</p><h3>Forventet sluttabell</h3></div><span className={`statusPill ${ownSavedCount===teamCount?"complete":""}`}>{ownSavedCount===teamCount?"✓ Lagret":"Ikke lagret"}</span></div><p className="muted tableTipHelp">Bruk pilene for å flytte lag. <b>1.</b> er laget du tror vinner serien.</p>
          ...
          <button className="primaryButton tableTipSave" disabled={!meId||locked||saving} onClick={save}>{locked?"🔒 Tabelltipset er låst":saving?"Lagrer …":ownSavedCount===teamCount?"Lagre endringer":"Lagre tabelltips"}</button>{!locked&&<p className="muted tableTipSaveHint">Du kan endre og lagre på nytt helt frem til fristen.</p>}
        </article>

        <article className="panel currentEhlTable"><div className="panelHeading"><div><p className="eyebrow">EHL akkurat nå</p><h3>Gjeldende tabell</h3></div><span className="statusPill">{standings.length}/{teamCount} lag</span></div>
