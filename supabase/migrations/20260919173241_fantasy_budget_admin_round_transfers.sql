-- Requested season budget increase and admin round allowance. Preserve transfer ledger and snapshots.
begin;
update public.fantasy_season_rules set budget=110.00,updated_at=now() where season='2026/27';
update public.fantasy_user_teams set budget=110.00,updated_at=now() where season='2026/27';

CREATE OR REPLACE FUNCTION public.apply_fantasy_transfers_v1(p_season text, p_name text, p_player_ids uuid[], p_captain uuid, p_vice_captain uuid)
 RETURNS TABLE(team_id uuid, transfer_batch_id uuid, effective_round_id uuid, effective_round_no integer, transfers_used integer, transfers_remaining integer, team_cost numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  select sr.max_players_per_club,sr.max_transfers_per_round,sr.budget
  into v_max_club,v_limit,v_budget
  from fantasy_season_rules sr where sr.season=p_season;
  if v_max_club is null or v_limit is null or v_budget is null then raise exception 'Fantasy season rules missing for %',p_season; end if;

  -- Trusted profile role; quota is per fantasy round, not per match.
  if p_season='2026/27' and exists(select 1 from public.players p where p.id=v_user and p.admin is true) then
    v_limit:=4;
  end if;

  v_base_limit:=v_limit;
  ...
  if v_cost>v_budget then raise exception 'Budget exceeded: %m > %m',v_cost,v_budget; end if;
  ...
  if v_booster.id is not null and v_booster.status='selected' and v_new_used>v_base_limit then
...
CREATE OR REPLACE FUNCTION public.get_fantasy_transfer_status_v1(p_season text)
 RETURNS TABLE(team_id uuid, effective_round_id uuid, effective_round_no integer, deadline_at timestamp with time zone, max_transfers_per_round integer, transfers_used integer, transfers_remaining integer, team_cost numeric, permanent_transfers_allowed boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
declare
  ...
  v_budget numeric;
  v_max_club integer;
  v_club text; v_club_count integer;
begin
  if v_user is null then raise exception 'Not authenticated'; end if;
  ...
  select max_players_per_club,budget into v_max_club,v_budget from fantasy_season_rules where season=p_season;
  if v_max_club is null or v_budget is null then raise exception 'Fantasy season rules missing for %',p_season; end if;

  select count(*) filter(where fp.position in('C','W')),
         count(*) filter(where fp.position='D'),
         count(*) filter(where fp.position='G'),
         coalesce(sum(sp.price),0)
  ...
  if v_f+v_d+v_g<>12 then raise exception 'One or more selected players are missing, inactive or have no locked season price'; end if;
  if v_f<>6 or v_d<>4 or v_g<>2 then raise exception 'Invalid roster: expected 6F/4D/2G, got %F/%D/%G',v_f,v_d,v_g; end if;
  if v_total>v_budget then raise exception 'Budget exceeded: %m > %m',v_total,v_budget; end if;
  ...
  values(v_user,p_season,coalesce(nullif(trim(p_name),''),'Mitt lag'),v_budget,now())
  ...
  with selected as (
    select fp.id,fp.position,sp.price,array_position(p_player_ids,fp.id) as ord,
      row_number() over (
        partition by case when fp.position in('C','W') then 'F' else fp.position end
        order by array_position(p_player_ids,fp.id)
      ) as pos_rank
    from fantasy_players fp
    join fantasy_player_season_prices sp on sp.player_id=fp.id and sp.season=p_season
    where fp.id=any(p_player_ids)
  ...
  select v_team,id,price,(id=p_captain),(id=p_vice_captain),
    case
      when position in('C','W') and pos_rank<=3 then 1
      when position='D' and pos_rank<=2 then 1
      when position='G' and pos_rank<=1 then 1
      else 2
    end
  from selected;
...
-- Obsolete writer bypasses round quotas and resets budget; current clients use v3.
revoke execute on function public.save_fantasy_team_v1(text,text,uuid[]) from public,anon,authenticated;
commit;
