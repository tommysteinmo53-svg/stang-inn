-- Stang Inn Fantasy Hockey – v0.21
-- Score immutable fantasy team snapshots against games in a calendar fantasy round.
-- Uses latest fantasy_player_points row per player/game.
-- Captain multiplier applies when captain has at least one game-stat row in the round.
-- Vice-captain inherits captain multiplier only when captain has NO game-stat row in the round.
...
using (
  exists (
    ...
    where trp.id=team_round_points_id
      and trp.user_id=auth.uid()
  )
);

-- Admin scorer for one fantasy round. Idempotent: recalculating replaces stored breakdown.
...
begin
  ...
  loop
    -- Captain is considered to have played if HockeyLive/stat import produced
    -- at least one fantasy_player_game_stats row for a game in this fantasy round.
    select exists(
      select 1
    ...
    loop
      -- Latest calculated fantasy_player_points version per player/game.
...
-- Admin overview of stored team scores for one round.
...
begin
  ...
  return query
  select
    trp.id,trp.snapshot_id,trp.team_id,trp.user_id,s.team_name,
...
-- Admin player breakdown for one stored team-round result.
create or replace function get_fantasy_team_round_player_breakdown_admin(
