-- Stang Inn Fantasy Hockey – v0.36
-- Extended public competition table for 2026/27.
-- Keeps get_fantasy_season_leaderboard() unchanged for backwards compatibility.
-- Adds participant count, latest round placement and season-rank movement since previous scored round.

create or replace function get_fantasy_competition_table_v1(
  p_season text
)
returns table(
...
as $$
  ...
  teams as (
    select
      ...
      lr.round_no as last_round_no,
      lr.total_points::numeric as last_round_points,
      lr.round_rank as last_round_position,
      lr.round_participants as last_round_participants
  ...
  current_ranked as (
    select
      teams.*,
      dense_rank() over(order by teams.total_points desc) as current_position,
  ...
  previous_ranked as (
    select
      pt.team_id,
      case
        when rm.previous_round_no is null then null::bigint
        else dense_rank() over(order by pt.previous_total desc)
      end as previous_position
    from previous_totals pt
    cross join round_markers rm
  )
  select
...
revoke all on function get_fantasy_competition_table_v1(text) from public;
grant execute on function get_fantasy_competition_table_v1(text) to authenticated;

comment on function get_fantasy_competition_table_v1(text) is
  'Extended Fantasy leaderboard: current/previous season rank, movement, participant count and latest round placement.';
