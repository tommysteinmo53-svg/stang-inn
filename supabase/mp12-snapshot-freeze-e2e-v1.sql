...
begin
  ...
  select count(*),
    count(*) filter(where position in('C','W')),
    count(*) filter(where position='D'),
    count(*) filter(where position='G'),
    count(*) filter(where line_no=1),
    count(*) filter(where line_no=2)
  into v_count,v_f_count,v_d_count,v_g_count,v_l1,v_l2
  from fantasy_team_round_snapshot_players where snapshot_id=v_snapshot;
  ...
  return query select 3,'Snapshot preserves captain and vice'::text,
    ...
    and exists(select 1 from fantasy_team_round_snapshot_players where snapshot_id=v_snapshot and player_id=v_vice and is_vice_captain and not is_captain),
    format('captain=%s vice=%s',v_cap,v_vice);
  ...
  delete from fantasy_user_teams where season=v_season;
  delete from fantasy_rounds where season=v_season;
  return query select 4,'Synthetic snapshot fixtures cleaned'::text,
    not exists(select 1 from fantasy_user_teams where season=v_season)
