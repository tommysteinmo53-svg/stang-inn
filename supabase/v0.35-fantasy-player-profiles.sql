-- Stang Inn Fantasy Hockey – v0.35
-- Read-only player profile aggregate for 2026/27.
-- Exposes public Fantasy profile data to authenticated users without opening other users' team rows.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'opponent',case when q.home_team=v_player.team then q.away_team else q.home_team end,
    'home',q.home_team=v_player.team,
    'roundNo',q.round_no,
    'status',q.status
  ...
  from (
    ...
    where g.season=p_season
      and (g.home_team=v_player.team or g.away_team=v_player.team)
      and g.starts_at>now()
...
comment on function get_fantasy_player_profile_v1(uuid,text) is
  'Read-only Fantasy player profile aggregate: fixed price, season stats, form, ownership, fixtures and fantasy-point history.';
