-- Stang Inn Fantasy Hockey – v0.28.2
-- Final transfer + fixed-price + lineup package for 2026/27.
-- Rules:
--   * maximum 2 player transfers per fantasy round
--   * published 2026/27 prices are fixed for the whole season
--   * captain/vice-captain changes are free
--   * 1st/2nd line changes are free and do not change roster membership
--   * each line is 3F/2D/1G
...
begin
  ...
  select v_team,fp.id,sp.price,(fp.id=p_captain),(fp.id=p_vice_captain),1
  from fantasy_players fp
  join fantasy_player_season_prices sp on sp.player_id=fp.id and sp.season=p_season
  where fp.id=any(p_player_ids);
...
begin
  ...
  if v_count<>6 then raise exception 'First line must contain exactly 6 players, got %',v_count; end if;

  select count(distinct x) into v_distinct from unnest(p_line1_player_ids) x;
  if v_distinct<>6 then raise exception 'Duplicate players are not allowed in first line'; end if;
