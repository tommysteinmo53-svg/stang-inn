...
begin
  if exists (
    select 1 from public.fantasy_private_leagues f
    join public.hockeytips_private_leagues h on h.id=f.id
    where (f.season,f.name,f.invite_code,f.created_by,f.created_at)
      is distinct from (h.season,h.name,h.invite_code,h.created_by,h.created_at)
  ) then raise exception 'MP-13.6 blocked: conflicting legacy league ids'; end if;

  if exists (
    select 1 from public.fantasy_private_leagues f
    join public.hockeytips_private_leagues h on h.invite_code=f.invite_code
    where h.id<>f.id
  ) then raise exception 'MP-13.6 blocked: conflicting legacy invite codes'; end if;
end $$;
...
on conflict(league_id,user_id) do update set
  role=case when excluded.role='owner' or stang_inn_private_league_members.role='owner' then 'owner' else 'member' end,
...
on conflict(source_product,source_league_id) do update set canonical_league_id=excluded.canonical_league_id,invite_code=excluded.invite_code;
insert into public.stang_inn_private_league_migration_audit(source_product,source_league_id,canonical_league_id,invite_code)
...
begin
  if exists (
    (select id,season,name,invite_code,created_by,created_at from public.fantasy_private_leagues except select id,season,name,invite_code,created_by,created_at from public.stang_inn_private_leagues)
    union all
    (select id,season,name,invite_code,created_by,created_at from public.hockeytips_private_leagues except select id,season,name,invite_code,created_by,created_at from public.stang_inn_private_leagues)
  ) then raise exception 'MP-13.6 migration verification failed: missing legacy league'; end if;

  if exists (
    (select league_id,user_id from public.fantasy_private_league_members except select league_id,user_id from public.stang_inn_private_league_members)
    union all
    (select league_id,user_id from public.hockeytips_private_league_members except select league_id,user_id from public.stang_inn_private_league_members)
  ) then raise exception 'MP-13.6 migration verification failed: missing legacy membership'; end if;

  if exists (
    select 1 from public.stang_inn_private_leagues l
  ...
  ) then raise exception 'MP-13.6 migration verification failed: owner invariant'; end if;
end $$;

create or replace function public.create_stang_inn_private_league_v1(p_season text,p_name text)
returns uuid language plpgsql security definer set search_path to 'public' as $$
declare uid uuid:=auth.uid(); league_id uuid; code text; clean_name text:=btrim(coalesce(p_name,''));
begin
  ...
  insert into public.stang_inn_private_leagues(season,name,invite_code,created_by) values(p_season,clean_name,code,uid) returning id into league_id;
  insert into public.stang_inn_private_league_members(league_id,user_id,role) values(league_id,uid,'owner');
  return league_id;
end $$;

create or replace function public.join_stang_inn_private_league_v1(p_season text,p_invite_code text)
returns uuid language plpgsql security definer set search_path to 'public' as $$
declare uid uuid:=auth.uid(); league_id uuid; clean_code text:=upper(btrim(coalesce(p_invite_code,'')));
begin
  if uid is null then raise exception 'Not authenticated'; end if;
  if clean_code='' then raise exception 'Invitasjonskode mangler'; end if;
  select l.id into league_id from public.stang_inn_private_leagues l where l.season=p_season and l.invite_code=clean_code;
  if league_id is null then raise exception 'Fant ingen miniliga med denne koden'; end if;
  insert into public.stang_inn_private_league_members(league_id,user_id,role) values(league_id,uid,'member') on conflict(league_id,user_id) do nothing;
  return league_id;
end $$;
...
begin
  if uid is null then raise exception 'Not authenticated'; end if;
  select m.role into member_role from public.stang_inn_private_league_members m
...
begin
  if auth.uid() is null then raise exception 'Not authenticated'; end if;
  if not exists(select 1 from public.stang_inn_private_league_members m join public.stang_inn_private_leagues l on l.id=m.league_id where m.league_id=p_league_id and m.user_id=auth.uid() and l.season=p_season) then raise exception 'You are not a member of this private league'; end if;
  return query
  with members as (
    select m.user_id,m.role,m.joined_at,coalesce(nullif(btrim(p.display_name),''),'Ukjent spiller')::text profile_name
    ...
    select m.user_id,m.profile_name display_name,b.team_id,coalesce(b.team_name,'Ikke opprettet lag')::text team_name,
      coalesce(b.total_points,0)::numeric total_points,coalesce(b.rounds_scored,0)::bigint rounds_scored,
      coalesce(b.round_wins,0)::bigint round_wins,coalesce(b.best_round_points,0)::numeric best_round_points,
      coalesce(b.average_round_points,0)::numeric average_round_points,b.last_round_no,b.last_round_points,m.role,m.joined_at
    from members m left join board b on b.user_id=m.user_id
  )
  select dense_rank() over(order by c.total_points desc,c.round_wins desc,c.best_round_points desc),c.user_id,c.display_name,c.team_id,c.team_name,c.total_points,c.rounds_scored,c.round_wins,c.best_round_points,c.average_round_points,c.last_round_no,c.last_round_points,c.role,c.joined_at
  from combined c order by c.total_points desc,c.round_wins desc,c.best_round_points desc,c.team_name,c.user_id;
end $$;
...
begin
  ...
  return query
  with members as (
    select m.user_id,m.role,m.joined_at,coalesce(nullif(btrim(p.display_name),''),'Hockeytipper')::text display_name
    from public.stang_inn_private_league_members m
    join public.stang_inn_private_leagues l on l.id=m.league_id and l.season=p_season
    left join public.players p on p.id=m.user_id and p.profile_name_confirmed_at is not null
    where m.league_id=p_league_id
    ...
    select mb.user_id,mb.display_name,mb.role,mb.joined_at,
      coalesce(sum(case when t.id is null or not coalesce(ma.finished,false) or ma.home_score is null or ma.away_score is null then 0 when t.points is not null then t.points when t.home_tip=ma.home_score and t.away_tip=ma.away_score then 5 when (t.home_tip>t.away_tip and ma.home_score>ma.away_score) or (t.home_tip<t.away_tip and ma.home_score<ma.away_score) or (t.home_tip=t.away_tip and ma.home_score=ma.away_score) then 3 else 0 end),0)::bigint total_points,
      count(*) filter(where coalesce(ma.finished,false) and ma.home_score is not null and ma.away_score is not null and t.home_tip=ma.home_score and t.away_tip=ma.away_score)::bigint exact_results,
      count(*) filter(where coalesce(ma.finished,false) and ma.home_score is not null and ma.away_score is not null and not(t.home_tip=ma.home_score and t.away_tip=ma.away_score) and ((t.home_tip>t.away_tip and ma.home_score>ma.away_score) or (t.home_tip<t.away_tip and ma.home_score<ma.away_score) or (t.home_tip=t.away_tip and ma.home_score=ma.away_score)))::bigint correct_outcomes,
      count(*) filter(where coalesce(ma.finished,false) and ma.home_score is not null and ma.away_score is not null and t.id is not null)::bigint scored_tips
    from members mb left join public.tips t on t.player_id=mb.user_id left join public.matches ma on ma.id=t.match_id
  ...
  from scored s order by s.total_points desc,s.exact_results desc,s.correct_outcomes desc,s.display_name;
end $$;

-- Compatibility RPCs. Existing route contracts stay intact while membership is canonical.
...
create or replace function public.get_fantasy_private_league_standings_v1(p_league_id uuid,p_season text)
returns table(standings_position bigint,user_id uuid,display_name text,team_id uuid,team_name text,total_points numeric,rounds_scored integer,round_wins integer,best_round_points numeric,average_round_points numeric,last_round_no integer,last_round_points numeric,member_role text,joined_at timestamptz)
language sql stable security definer set search_path to 'public' as $$
select standings_position,user_id,display_name,team_id,team_name,total_points,rounds_scored::integer,round_wins::integer,best_round_points,average_round_points,last_round_no,last_round_points,member_role,joined_at
from public.get_stang_inn_private_league_fantasy_standings_v1(p_league_id,p_season)$$;
...
-- Legacy product tables stay as immutable migration history.
