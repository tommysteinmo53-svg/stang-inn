-- MP-10 – authenticated read-only xFP input for the end-user optimizer.
-- Keeps all admin analysis RPCs unchanged. Returns only player projection data.
-- No user-team data is exposed here and this function performs no writes.
...
revoke all on function public.get_fantasy_xfp_round_horizons_v1(text) from public;
revoke all on function public.get_fantasy_xfp_round_horizons_v1(text) from anon;
grant execute on function public.get_fantasy_xfp_round_horizons_v1(text) to authenticated;
comment on function public.get_fantasy_xfp_round_horizons_v1(text) is
  'MP-10 read-only authenticated player xFP horizons for end-user optimizer. No private user-team data and no writes.';
