...
export default function HomeFantasyLineup({teamId}:{teamId:string|null}){
 ...
 const visible=line===1?line1:line2,bench=line===1?line2:line1;
 ...
 return <div className={styles.wrap}><div className={styles.tabs} role="tablist" aria-label="Velg rekke"><button type="button" role="tab" aria-selected={line===1} className={line===1?styles.active:""} onClick={()=>setLine(1)}>1. rekke</button><button type="button" role="tab" aria-selected={line===2} className={line===2?styles.active:""} onClick={()=>setLine(2)}>2. rekke</button></div><div className={styles.rink} aria-label={`${line}. rekke`}><div className={`${styles.blueLine} ${styles.blueLeft}`}/><div className={`${styles.blueLine} ${styles.blueRight}`}/><div className={styles.centerCircle}/><div className={styles.crease}/>{positioned.map(({player,slot})=><a href={`/fantasy/players/${player.id}`} className={`${styles.card} ${slot}`} key={player.id}><span className={styles.jersey}><span className={styles.jerseyMark}>{group(player)}</span>{player.captain&&<b className={styles.badge}>C</b>}{player.vice&&<b className={styles.badge}>VC</b>}</span><span className={styles.name}><strong>{player.name}</strong><small>{player.team} · {player.price.toFixed(1)}m</small></span></a>)}</div><div className={styles.bench}><div className={styles.benchHead}><strong>{line===1?"2. rekke":"1. rekke"}</strong><span>{bench.length}/6 spillere</span></div><div className={styles.benchGrid}>{bench.map(player=><a href={`/fantasy/players/${player.id}`} className={styles.benchCard} key={player.id}><span className={styles.benchJersey}>{group(player)}</span><strong>{player.name}</strong><small>{player.team} · {player.price.toFixed(1)}m</small></a>)}</div></div></div>;
}
