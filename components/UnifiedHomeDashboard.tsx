...
export default function UnifiedHomeDashboard(){
 ...
 const[matches,setMatches]=useState<Match[]>([]),[tips,setTips]=useState<Tip[]>([]),[tippingSummary,setTippingSummary]=useState<TippingSummary|null>(null),[fantasy,setFantasy]=useState<Fantasy>(initialFantasy),[leagues,setLeagues]=useState<League[]>([]);
 ...
 useEffect(()=>{if(pathname!=="/")return;(async()=>{try{
  ...
  setMatches((m||[])as Match[]);setTips((t||[])as Tip[]);setLeagues((l||[])as League[]);
 ...
 const tippingRank=tippingSummary?.standings_position||null;
 const myTipPoints=tippingSummary?.points||0;
 ...
 return <main className="unifiedHomeDashboard">
  <section className="unifiedHero">
   <div className="unifiedHeroRound"><div className="unifiedHeroPlayer" aria-hidden="true"><span className="unifiedHeroPlayerHalo"/><span className="unifiedHeroPlayerIcon"><SIIcon name="player" size={210}/></span><span className="unifiedHeroStick"/><span className="unifiedHeroPuck"/></div><div className="unifiedHeroCopy"><p className="unifiedEyebrow">NESTE FANTASYRUNDE</p><h1>{fantasy.roundNo?<>RUNDE <em>{fantasy.roundNo}</em></>:"EHL 2026/27"}</h1><p>{fantasy.deadline?`Deadline: ${fmt(fantasy.deadline)}`:"Neste Fantasy-deadline publiseres her."}</p><strong className="unifiedCountdown">{timeLeft(fantasy.deadline)}</strong><a href="/fantasy/rounds">Gå til runde →</a></div></div>
  ...
  <section className="unifiedSecondaryGrid">
   <article className="unifiedPanel"><div className="unifiedPanelHead"><div><p className="unifiedEyebrow">FANTASY</p><h2>Rundehistorikk</h2></div><a href="/fantasy/leaderboard">Se all historikk →</a></div><HomeRoundHistory teamId={fantasy.teamId}/></article>
  ...
  <section className="unifiedSecondaryGrid">
   ...
   <article className="unifiedPanel"><div className="unifiedPanelHead"><div><p className="unifiedEyebrow">DIN AKTIVITET</p><h2>Siste hendelser</h2></div></div><HomeActivityFeed teamId={fantasy.teamId}/></article>
  </section>
